...
{    
   ...
   $('.merchant_delivery_type').click(function()
   {
       if($(this).val()==0)
       {
          $('#delivery_fee').val('');
       ...
       {
        $('#delivery_fee').val($('#hidden_delivery_fee').val());
       }      
   });
...
//  var item_array = new Array(); merchant_delivery_type
 


  $('.deliver_all_parish').on('ifChecked', function(event)
  {     
        $('.delivery_charges_table').css('display','none');
  });

  $('.deliver_all_parish').on('ifUnchecked', function(event)
  {    
        $('.delivery_charges_table').css('display','block');
  });

  $('#minimum_order_req').on('ifChecked', function(e) 
  {     
       $('.minimum_order_amount_row').css('display','block');
  });

  $('#minimum_order_req').on('ifUnchecked', function(e) 
  {     
       $('.minimum_order_amount_row').css('display','none');
  });


  $('.parish_delivery_fee').on('click', function() 
  { 
    var input_id      = ($(this).attr("id")).split("_");
    var delivery_type = $(this).attr("value");
    if(delivery_type==0)
    {
      $('.parish_delivery_amt_'+input_id[3]).css('display','none');
      $('#parish_min_amt_div_'+input_id[3]).css('display','none');
      $('#delivery_fee_'+input_id[3]).val('');
    }
    else
    {
      $('.parish_delivery_amt_'+input_id[3]).css('display','block');
      $('#parish_min_amt_div_'+input_id[3]).css('display','block');
    }
    
  });


  $('.terms_type').on('click',function()
  {
    var selected_type = $(this).val();
    if(selected_type==0)
    {
      $('#txtbox_terms_condition').css('display','none');
      $('#url_terms_condition').css('display','block');
    }
    if(selected_type==1)
    {
      $('#url_terms_condition').css('display','none');
      $('#txtbox_terms_condition').css('display','block');
    }
  });


  /* $('.minimum_order_req_parish').on('click', function() 
  { 
    var input_id      = ($(this).attr("id")).split("_");
    var delivery_type = $(this).attr("value");
    if(delivery_type==0)
  ...
  $('.minimum_order_req_parish').on('change', function() {
    ...
    if(val=='')
    {
      $('#parish_min_amt_'+input_id[4]).css('display','none');
      $('#delivery_fee_'+input_id[4]).css('display','none');
      $('#delivery_fee_'+input_id[4]).val('');
    }
    else
    {
      $('#parish_min_amt_'+input_id[4]).css('display','block');
      $('#delivery_fee_'+input_id[4]).css('display','block');
    }
    
});

  $('#item').on('change', function(e, params) 
  { 
    var item_id = $.trim(params.selected);          
    if((item_id!='undefined')&&(item_id.length!=0))
    {
      var single_item = $('#item option[id="'+params.selected+'"]').attr('single_item');
      if($.trim(single_item)==1)
      {
        var params="action=viewSelectDealsItem&currentController=merchant&item_id="+item_id+"&tbl=viewFoodItem";        
         open_modal_box(params);
      }
      else
      {
        var params="action=viewSelectsingleDealsItem&currentController=merchant&item_id="+item_id+"&tbl=viewFoodItem";        
        var bogo_multi_size_val = $('#bogo_multi_size').val();
         busy(true);
        $.ajax({    
        type   : "POST",
        url    : ajax_url,
        ...
        {            
          if(bogo_multi_size_val!='')
          {
            $('#bogo_multi_size').val(bogo_multi_size_val+"___"+data);  
          }          
          else
          {
            if(data!='')
            {
              $('#bogo_multi_size').val(data);  
            }            
          }
  ...
  $('body').on('click', '.add_bogo_multi_size', function() 
  {
    var item_id      = $('#item_id').val();
    // var size_details = $('input[class=item_price]:checked', '#frm-fooditem').val();
    ...
    var initail_count = 1 ;  
      $("input[type=checkbox]:checked").each(function() {
        ...
        initail_count = parseInt(initail_count)+1;
    });   
     
    var merchant_id  = $('#merchant_id').val();
    /* var size         = size_details.split('|');   
    size             = size[1]; */
    var size         = item_list; 
    var ajax_url     = $('#base_url').val()+"/merchant/add_bogo_multi_size";
    var bogo_multi_size_val = $('#bogo_multi_size').val();
    busy(true);
    $.ajax({    
      type   : "POST",
      url    : ajax_url,
      data   : {item_id:item_id,size:size,merchant_id:merchant_id},      
      success: function(data){ 
...
function open_modal_box(params)
  {     
    ...
    $.ajax({    
      ...
      success: function(data){ 
        ...
        // $('#myModal_add_cart').modal({backdrop: 'static', keyboard: false});  
        //alert(data);  
      }, 
      error: function(){                  
        busy(false); 
  ...
  function change_sts(sts,id)
  {
    ...
    var url = result+"/merchant/update_status";
    var html = '';
    busy(true);
    $.ajax({    
      type   : "POST",
      ...
      success: function(data){ 
        ...
        if(data==0)
        {
          ...
          if(sts==1)
          {
            html = '<img class="uk-thumbnail uk-thumbnail-mini" src="/assets/images/deactive.png"><a href="javascript:;" onclick="change_sts(0,'+id+')"> Activate </a>'; 
          }
          else
          {
            html = '<img class="uk-thumbnail uk-thumbnail-mini" src="/assets/images/active.png"><a href="javascript:;" onclick="change_sts(1,'+id+')"> Deactivate </a>'; 
          }
          $('#sts_div_'+id).html(html);
        }       
      }, 
      error: function(){                  
        busy(false); 
	...
	//alert(params); exit();
	 ...
	 $.ajax({    
        ...
        success: function(data){ 
            ...
            if(action=="merchant_add_deals")
            {
              // window.location.href=data.details; 
              window.location.href = $.trim(window.location.protocol + "//" + window.location.host + "/merchant/deals");
            }


            if(action=="bookingAlertSettings")
            {
            ...
            if(action=="parish_delivery_prices")
            {              
...
jQuery(document).ready(function() {
   ...
   $( document ).on( "change", ".merchant_enable_offer", function() {      
        if(this.checked) {
        $('#merchant_deals_div').css('display','block');                
    }
    else
    {
      $('#merchant_deals_div').css('display','none');                
      $('#deal_title').val('');
      $('.jqte_editor').html('');
      $('#from_date').val('');
      $('#to_date').val('');
   ...
   $(document).on('click','.select_deal',function()
   {
      var selected_option = $(this).val();
      if(selected_option==0)
      {
        $('#buy_one_get_one').css('display','block');
        $('#spend_amount').css('display','none');
        $('#spend_amount').val('');
        $('#discount_deals').css('display','none');
      ...
      if(selected_option==1)
      {
        $('#buy_one_get_one').css('display','block');
        $('#item').val('');
        $('#spend_amount').css('display','block');
        $('#discount_deals').css('display','none');
        $('#discount_percent').val('');
      }
      if(selected_option==2)
      {
        $('#buy_one_get_one').css('display','none');
        $('#item').val('');
        $('#spend_amount').css('display','block');
        $('#discount_deals').css('display','block');
      }
   });

   $( document ).on( "click", ".deals_radio_opn", function() {      
        if(parseInt(this.value)==2)
        {
          $('#discount_input').css('display','block');
        }
        else
        {
          $('#discount_input').css('display','none');
          $('#discount').val('');
        }
   });
  ...
  $(".add_table_menus").on('change', function() 
  {
    if ($(this).is(':checked')) 
    {
    ...
    if( jQuery('#Inhouse_gallery').is(':visible') ) {     
       createUploader('Inhouse_gallery','Inhouse_gallery');
    }       

    if( jQuery('#spl_gallery').is(':visible') ) {     
       createUploader('spl_gallery','spl_gallery');
    }       

     if( jQuery('#bgimage').is(':visible') ) {      
       createUploader('bgimage','bgimage');
    }  
    ...
    $('.numeric_only').keyup(function () {     
      ...
      if($('.merchant_delivery_estimation').exists())
      {
          var total_mins = parseInt($('.merchant_delivery_estimation').val());
          if(total_mins!='')
          {
                ...
                $('#delivery_estimation_in_hrs_mins').html(hours+minutes);
          }
      }

          var total_mins = parseInt($('.merchant_pickup_estimation').val());
          if(total_mins!='')
          {
                var hours = Math.floor( total_mins / 60);          
                var minutes = total_mins % 60;
                ...
                $('#pickup_estimation_in_hrs_mins').html(hours+minutes);
          }      
...
var maxDate =  new Date(currentTime.getFullYear(), currentTime.getMonth() +3, +0);
   ...
   jQuery(".j_date_table_booking").datepicker( { 
       dateFormat: website_date_picker_format,        
       altFormat: "yy-mm-dd",       
       changeMonth: true, 
       changeYear: true ,      
    ...
    onSelect : function( element, object ) {
      ...
      dump(formatedDate);
      $("#"+original_id).val(formatedDate);   
      set_table_booking_date(formatedDate);
    }
  });    
...
function set_table_booking_date(currentDate){
       var ajax_url = $('#base_url').val()+'/merchant/setNewtableBookingtime';              
       ...
       $.ajax({    
            ...
            success: function(data){    
              ...
              {
                $('#table_body').html('');  
                $('#table_booking_table').css('display','block');
...
{       
  ...
  $(".preview").show();
  img+="<img src=\""+upload_url+"/"+data.details.file+"\" alt=\"\" title=\"\" class=\"uk-thumbnail uk-thumbnail-mini\" >";
  img+="<input type=\"hidden\" name=\"bgimage\" value=\""+data.details.file+"\" >";
  img+="<p><a href=\"javascript:rm_preview();\">"+js_lang.removeFeatureImage+"</a></p>";
  $(".image_preview").html(img);
}
...
function spl_gallery(data)
{ 
  var img='';
  $(".preview").show();
  img+='<li>';
  img+="<img src=\""+upload_url+"/"+data.details.file+"\" alt=\"\" title=\"\" class=\""+data.details.id+" uk-thumbnail uk-thumbnail-mini\" >";
  img+="<input type=\"hidden\" name=\"spl_photo[]\" value=\""+data.details.file+"\" class=\""+data.details.id+"\" >";
  img+="<p class=\""+data.details.id+"\"><a href=\"javascript:rm_gallery('"+data.details.id+"');\">"+js_lang.removeFeatureImage+"</a></p>";
  img+='</li>';
  $(".spl_gallery_preview").append(img);
}
