...
jQuery(document).ready(function() {		
   ...
   $( document ).on( "click", ".menu-1 .menu-cat a", function() {
	...
	$( document ).on( "click", ".view-merchant-map", function() {	
		 ...
		 if ( !empty(merchant_information)){
			 resto_info+='<div class="marker-wrap map-wrap">';
		   	   resto_info+='<div class="row">';
			   	   resto_info+='<div class="col-md-3 ">';
			   	   resto_info+='<img class="logo-small img-responsive" src="'+merchant_information.merchant_logo+'" >';
			   	   resto_info+='</div>';
			   	   resto_info+='<div class="col-md-8 ">';
				   	   resto_info+='<h3 class="res-name-map">'+merchant_information.restaurant_name+'</h3>'; 
    ...
    $( document ).on( "click", ".view-full-map", function() {        		   
        ...
        if ( $(".full-map-wrapper").hasClass("full-map") ) {
        	...
        	$(".view-full-map").removeClass("btn-primary");
        	$(".view-full-map").addClass("btn-danger");
        } else {
        	$(this).html(js_lang.view_fullscreen);     	
        	$(".view-full-map").addClass("btn-primary");
        	$(".view-full-map").removeClass("btn-danger");
        }
    });    
   ...
   $( document ).on( "click", ".view-receipt-front", function() {    	       	 	
	    ...
	    var params="action=viewOrderDetails&currentController=store&order_id="+$(this).data("id")+"&tbl=viewFoodItem";
	    order_details_modal(params);
   });	
...
function order_details_modal(params)
  {  	  	  	  	
   	busy(true);
	$.ajax({    
		type   : "POST",
...
function initRating()
{
	...
	$('.rating-stars').find('img').each(function(index, element) {
    			   ...
    			   var bounds = [];
     			   $.each(data.details, function( index, val ) {
     			   	   ...
     			   	   resto_info+='<div class="marker-wrap map-wrap">';
	    			   	   resto_info+='<div class="row">';
		    			   	   resto_info+='<div class="col-md-3 ">';
		    			   	   resto_info+='<img class="logo-small img-responsive" src="'+val.logo+'" >';
		    			   	   resto_info+='</div>';
		    			   	   resto_info+='<div class="col-md-8 ">';
			    			   	   resto_info+='<h3 class="res-name-map">'+val.restaurant_name+'</h3>'; 
			    			   	   resto_info+='<p class="small">'+val.merchant_address+'</p>'; 
			    			   	   resto_info+='<a class="btn btn-primary btn-sm" href="'+val.link+'">'+js_lang.trans_37+'</a>';
			    			   resto_info+='</div>';
	    			   	   resto_info+='</div>';
    			   	   resto_info+='<div>';    		
...
function uploadAvatar(data)
{
	...
	if ( data.code==1){
		...
		$(".avatar-img-appnd").html( '<img src="'+upload_url+"/"+data.details.file +'" class="img-responsive img-circle" width="150" height="150" />' );
