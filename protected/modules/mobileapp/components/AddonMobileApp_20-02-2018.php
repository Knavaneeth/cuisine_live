...
class AddonMobileApp
{
	...
	public static function merchantInformation($merchant_id='')
	{
		...
		if ($merchantinfo=Yii::app()->functions->getMerchant($merchant_id))
		{
			...
			$data['open']=FunctionsV3::getMerchantCurrentStatus($merchant_id);		
	...
	public static function computeCart($data='')
	{
		...
		if (is_array($cart) && count($cart)>=1){
	        ...
	        if(isset($data['extra_delivery_fee']))
	        {
	        	$delivery_charges += $data['extra_delivery_fee'];	
	        }        
	...
	public static function getOrderDetails($order_id='')
	{
		...
		a.qty,
		a.normal_price,a.discounted_price,
		a.size,
		(
		  select total_w_tax 
		  from {{order}}
    ...
    public static function getAddressBook($client_id='')
    {
    	...
    	";*/    	    	
    	$stmt="SELECT  
    	       concat(location_name,'',street,' ',city,' ',state,' ',zipcode) as address,
    	       a.id,
    	       a.location_name,
    	       a.country_code,
    	       a.as_default,
    ...
    public static function hasDefaultAddress($client_id='')
    {
    	$db_ext=new DbExt;    	
    	$stmt="SELECT * FROM
    	{{address_book}}
    	...
    	{    		
    		...
    		if ($val['enabled_push']==1){
    			...
    			if($val['client_name']=='')
    			{
    				...
    				if ($client_name_res=$db_ext->rst($client_name_qry))
			    	{    		
			    		$val['client_name'] = $client_name_res[0]['client_name'];    		
			    	}		    	 
			    	else
			    	{
    ...
    {	
	    				...
	    				if (is_numeric($balance) && $balance>=1)
	    				{
            					...
            					$sms_provider=strtolower($sms_provider);
	    					 	...
	    					 	$merchant_sms_tpl = " Hi ".$client_fullname." Your Table booking on ".$params['date_booking']." for ".$params['number_guest']." is confirmed" ;
		    					$resp=Yii::app()->functions->sendSMS($merchant_number,$merchant_sms_tpl);
					        	$params=array(
					        	  'merchant_id'=>$merchant_id,
					        	  'broadcast_id'=>"999999999",
