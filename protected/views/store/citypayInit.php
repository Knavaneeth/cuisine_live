...
if ( $data=Yii::app()->functions->getOrder($_GET['id'])){     
	...
	$failure_redirect_url = Yii::app()->request->getBaseUrl(true).'/store/menu/merchant/'.$data['restaurant_slug'];   
  $postback_redirect_url = Yii::app()->getBaseUrl(true).'/PaymentOption/';
  if(isset($_SESSION['guest_checkout_cpy'])&&($_SESSION['guest_checkout_cpy']=="true"))
  {     
    if(isset($_SESSION['basket-url']))
    {
      $failure_redirect_url = $postback_redirect_url = Yii::app()->request->getBaseUrl(true).$_SESSION['basket-url'];
    }
    else
    {
      $failure_redirect_url = $postback_redirect_url = Yii::app()->request->getBaseUrl(true)."/searcharea?parish=0&zipcode=&filter_cuisine=";  
    }
    unset($_SESSION['guest_checkout_cpy']);
  }
 ...
 //  if ( !empty($citypay_con[$citypay_con['mode']]['user'])){   	     
      ...
      /*  echo 'MERCHANT_ID '.$city_pay_merchant_id.' LICENCE_KEY ', $city_pay_lisence_key ;
        ...
        $request['test'] = "false";
        if($citypay_con[$citypay_con['mode']]=="sandbox")
        {
          $request['test'] = "true";
        }
       ...
       /* $request['expmonth'] = "12" ;
        ...
        $request['config']['postback'] = $postback_redirect_url;
        ...
        $request['config']['redirect_failure'] = $postback_redirect_url;
