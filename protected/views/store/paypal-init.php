...
if ( $ok==TRUE){
  ...
  // print_r($_SESSION['kr_item']);
   // print_r($_SESSION['get_all_items']);
   echo "</pre>";
   
   	$manual_item = array();
  	if(isset($_SESSION['get_all_items'])&&!empty($_SESSION['get_all_items']))
   	{
   	   	$session_array = array();
		foreach ($_SESSION['get_all_items'] as $session_key => $session_value) 
		{
		 	if(is_numeric($session_key))
		 	{		 			 
		 		foreach($session_value as $manual_item_key=>$manual_item_formation)
		 		{
		 		   if($manual_item_formation['partition_type']=="normal")
		 		   {
		 		   		$manual_item[] = $manual_item_formation;
		 		   }
		 		}
		 	}
		}		
   ...
   if ( !empty($paypal_con[$paypal_con['mode']]['user'])){   	      
	   ...
	   if (is_array($data2) && count($data2)>=1)
	   {
		   ...
		   if (($data2['total']['discounted_amount']>0)||isset($_SESSION['kr_item']['total_discount'][0]['discount_price']))
		   {		   	
				$discount_total = $data2['total']['discounted_amount'];	
			   if(isset($_SESSION['kr_item']['total_discount'][0]['discount_price']))
		   	   {		   	   		 
		   	   		$discount_total =	$data2['total']['discounted_amount'] + $_SESSION['kr_item']['total_discount'][0]['discount_price'] ;
		   	   } 	   	    
		   	  /*  if(isset($_SESSION['kr_item']['total_discounted_amount']['discounted_amount']))
		   	   {		   	   		 
		   	   		$discount_total =	$data2['total']['discounted_amount'] + $_SESSION['kr_item']['total_discounted_amount']['discounted_amount'] ;
		   	   } */			   	    
		   	   $x++;
		  	   $params['L_NAME'.$x]=Yii::t("default","Discount");  	  	  
	  	  	   $params['L_AMT'.$x]="-".isIsset(Yii::app()->functions->prettyFormat($discount_total,$data['merchant_id']));
	  	  	   $params['L_QTY'.$x]=1;
		   }		

		   /* if(isset($_SESSION['kr_item']['total_discount'][0]['discount_price'])&&($_SESSION['kr_item']['total_discount'][0]['discount_price']>0))
		   {
		   	   $x++;
		  	   $params['L_NAME'.$x]=Yii::t("default","Deals Discount");  	  	  
	  	  	   $params['L_AMT'.$x]="-".isIsset( normalPrettyPrice($_SESSION['kr_item']['total_discount'][0]['discount_price']) );
	  	  	   $params['L_QTY'.$x]=1;
		   } */
		   ...
		   if(isset($_SESSION['kr_item']['total_discounted_amount']['discounted_amount']))
		   {
		   		$params['AMT']=isIsset($data2['total']['total'])?$data2['total']['total']+Yii::app()->functions->prettyFormat($_SESSION['kr_item']['total_discounted_amount']['discounted_amount'],$merchant_id):normalPrettyPrice($data2['total']['total']);		
		   }		   


		   if(isset($discount_total))
		   {
		   		if($discount_total>0)
		   		{ 			   			 
		   			$params['AMT'] = prettyFormat(($params['AMT']-Yii::app()->functions->prettyFormat($discount_total,$data['merchant_id'])),$merchant_id);
		   			// $params['AMT'] = (($params['AMT']-$discount_total));
		   		}
		   }
		   ...
		   {		   		
		   		if($less_voucher>0)
		   		{		   			 
		   			$params['AMT'] = prettyFormat(($params['AMT']-$less_voucher),$merchant_id);		   			 
		   		}
		   }

		   

		   if(isset($data2['total']['delivery_charges']))
		   {
			   	if($data2['total']['delivery_charges']>0)
			   	{
			   		$params['AMT'] +=$data2['total']['delivery_charges'];
			   	}
		   }
