<div class="row">
	<div class="col-sm-12">
		<div class="panel table-responsive">
			<div class="panel-heading">
				<div class="merchant-btns">
					<a href="<?php echo Yii::app()->request->baseUrl; ?>/admin/sponsoredMerchantList/Do/Add" class="btn btn-default"><i class="fa fa-plus"></i> <?php echo Yii::t("default","Add New")?></a>
					<a href="<?php echo Yii::app()->request->baseUrl; ?>/admin/sponsoredMerchantList" class="btn btn-default"><i class="fa fa-list"></i> <?php echo Yii::t("default","List")?></a>
				</div>
			</div>
			<div class="panel-body">
				<div class="label label-primary inline-block m-b-15"><?php echo Yii::t("default","List of merchant that will put on top of the search.")?></div>
				<h4 class="mt-0 header-title"><b><?php echo Yii::t("default","Merchant List")?></b></h4>
				<form id="frm_table_list" method="POST" >
					<input type="hidden" name="action" id="action" value="sponsoredMerchantList">
					<input type="hidden" name="tbl" id="tbl" value="merchantSponsoredList">
					<input type="hidden" name="clear_tbl"  id="clear_tbl" value="clear_tbl">
					<input type="hidden" name="whereid"  id="whereid" value="merchant_id">
					<input type="hidden" name="slug" id="slug" value="sponsoredMerchantList">
					<table id="table_list" class="table table-striped table-bordered">
					   <thead>
							<tr>
								<th><?php echo Yii::t("default","MerchantID")?></th>
								<th><?php echo Yii::t("default","Merchant Name")?></th>
								<th><?php echo Yii::t("default","Expiration Date")?></th>            
							</tr>
						</thead>
						<tbody>
						</tbody>
					</table>
				</form>
			</div>
		</div>
	</div>
</div>