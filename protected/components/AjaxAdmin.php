...
if (!class_exists('AjaxAdmin'))
{
	class AjaxAdmin extends DbExt
	{
		...
		{			
			...
			$params['parish']=isset($this->data['parish'])?$this->data['parish']:'';		    
		    ...
		    if (empty($this->data['id'])){	
		    	if ( $this->insertData("{{merchant}}",$params)){
		    		...
		    		$this->insertData("{{option}}",array('merchant_id'=>$this->details,'option_name'=>'merchant_table_booking','option_value'=>'yes'));
		    		$this->code=1;
		...
		public function rowDelete()
		{
			...
			if ($this->data['tbl']=="merchant"){
				...
				if ( $functionk->getMerchantOrders($this->data['row_id'])){
					$this->msg=t("Sorry! you cannot delete this merchant it has reference on order tables");
					return ;
				}			
			}			 
		...
		public function subcategoryList()
		{
			...
			$slug=$this->data['slug'];
			$stmt="
			 	SELECT mt_subcategory_menu.*,mt_category.category_name FROM `mt_subcategory_menu`
				LEFT JOIN mt_category ON mt_category.cat_id = mt_subcategory_menu.cat_id
				WHERE mt_subcategory_menu.merchant_id  ='".Yii::app()->functions->getMerchantID()."'
    	    ...
    	    if (is_array($rows) && count($rows)>=1){
    	    	foreach ($rows as $val) {    	 
    	    		$chk="<input type=\"checkbox\" name=\"row[]\" value=\"$val[sub_cat_id]\" class=\"chk_child\" >";   		
    	    		$option="<div class=\"options\">
    	    		<a href=\"$slug/id/$val[sub_cat_id]\" >".Yii::t("default","Edit")."</a>
    	    		<a href=\"javascript:;\" class=\"row_del\" rev=\"$val[sub_cat_id]\" >".Yii::t("default","Delete")."</a>
    	    		</div>";
    	    		...
    	    		$feed_data['aaData'][]=array(
    	    		  ...
    	    		  stripslashes($val['category_name']),    	    		      	    		  
    	    		  $date."<div>".Yii::t("default",$val['status'])."</div>"
    	    		);
    	    	}
    	    	$this->otableOutput($feed_data);
		...
		public function deliveryboysList()
		{
			$mtid=Yii::app()->functions->getMerchantID();
			$slug=$this->data['slug'];
			$stmt=" SELECT * FROM `mt_delivery_boys` WHERE `merchant_id` = ".$mtid." ORDER BY driver_name ASC ";
			$connection=Yii::app()->db;
    	    $rows=$connection->createCommand($stmt)->queryAll();     	    
    	    if (is_array($rows) && count($rows)>=1){
    	    	foreach ($rows as $val) {    	 
    	    		...
    	    		// $status_link  = '<a href="javascript:;" onclick="change_sts(\''active'\','.$val['id'].')" > Activate </a>';
    	    		// $status_link  = '<a href="javascript:;" onclick= "change_sts(\''deactive'\','.$val['id'].')" > Deactivate </a>';

    	    		$status_image = Yii::app()->request->baseUrl."/assets/images/deactive.png";
    	    		$status_link  = '<a href="javascript:;" onclick="change_sts(0,'.$val['id'].')" > Activate </a>';
    	    		if($val['status']=="active")
    	    		{
    	    			$status_image = Yii::app()->request->baseUrl."/assets/images/active.png";
    	    			 $status_link  = '<a href="javascript:;" onclick= "change_sts(1,'.$val['id'].')" > Deactivate </a>';
    	    		}

    	    		$sts_img_src = "<img class=\"uk-thumbnail uk-thumbnail-mini\" src=\"$status_image\" >";

    	    		$feed_data['aaData'][]=array(
    	    		  $chk,stripslashes($val['driver_name']).$option,
    	    		  stripslashes($val['mobile_no']),    
    	    		  stripslashes($val['mailid']), 
    	    		  stripslashes($val['driving_license_no']),    	    		      	    		  
    	    		  $date,
    	    		  "<div id='sts_div_".$val['id']."'>".$sts_img_src.$status_link."</div>"
    	    		);
    	    	}
    	    	$this->otableOutput($feed_data);
    	    }     	    
		...
		public function tablebookingexceptionList()
		{
			$mtid=Yii::app()->functions->getMerchantID();
			$slug=$this->data['slug'];
			$stmt=" SELECT * FROM `mt_table_booking_exception` WHERE `merchant_id` = ".$mtid." ORDER BY booked_date ASC ";
			$connection=Yii::app()->db;
    	    $rows=$connection->createCommand($stmt)->queryAll();     	    
    	    if (is_array($rows) && count($rows)>=1){
    	    	foreach ($rows as $val) {    	 
    	    		...
    	    		$date=FormatDateTime($val['booked_date']);
    	    		
    	    		 
    	    		$feed_data['aaData'][]=array(
    	    		  stripslashes($val['customer_name']).$option,
    	    		  stripslashes($val['customer_mobile']),    
    	    		  stripslashes($val['occasion_type']), 
    	    		  stripslashes($val['no_of_seats']),    	    		      	    		  
    	    		  $date    	    		   
    	    		);
    	    	}
    	    	$this->otableOutput($feed_data);
	    ...
	    {	    	
		    ...
		    if (isset($this->data['qqfile']) && !empty($this->data['qqfile'])){
	            ...
	            if($this->data['gallery_type']=="Inhouse_gallery"||$this->data['gallery_type']=="spl_gallery")
		        {
		        	$this->makeThumbnails($path_to_upload,$file_name,'');
		        }		         
		...
		{
			$params=array(
			  'sub_category_name'=>addslashes($this->data['subcategory_name']),		
			  'sequence'=>0,	  
			  'cat_id'=>$this->data['category_name'],
			  'sub_category_desc'=>$this->data['subcategory_desc'],	
			  'status'=>addslashes($this->data['status']),
			  'date_created'=>date('c'),
			...
			{				
				unset($params['date_created']);
				$params['date_modified']=date('c');				
				$res = $command->update('{{subcategory_menu}}' , $params , 
				'sub_cat_id=:sub_cat_id' , array(':sub_cat_id'=> addslashes($this->data['id']) ));
				if ($res){
					$this->code=1;
	                $this->msg=Yii::t("default",'Category updated.');  
				} else $this->msg=Yii::t("default","ERROR: cannot update");
				...
				if ($res=$command->insert('{{subcategory_menu}}',$params))
				{
					$this->details=Yii::app()->db->getLastInsertID();	                
	                $this->code=1;
	                $this->msg=Yii::t("default",'Category added.');  	                
		...
		public function addDeliveryboys()
		{

			$params=array(
			  'driver_name'=>addslashes($this->data['driver_name']),		
			  'merchant_id'=>Yii::app()->functions->getMerchantID(),			  
			  'mobile_no'=>$this->data['driver_mobile'],
			  'driving_license_no'=>$this->data['driver_license'],
			  'mailid'=>$this->data['driver_email'],				  
			  'status'=>addslashes($this->data['status']),
			  'date_created'=>date('c')
			  );				
			...
			{				
				 unset($params['date_created']);				
				$res = $command->update('{{delivery_boys}}' , $params , 
				...
				if ($res=$command->insert('{{delivery_boys}}',$params))
				{
					$this->details=Yii::app()->db->getLastInsertID();	                
	                $this->code=1;
	                $this->msg=Yii::t("default",'Driver added.');  	                
	            } else $this->msg=Yii::t("default",'ERROR. cannot insert data.');
			}	
		...
		public function addTableBookingException()
		{

			$params=array(
			  'customer_name'=>addslashes($this->data['customer_name']),		
			  'merchant_id'=>Yii::app()->functions->getMerchantID(),			  
			  'customer_mobile'=>$this->data['customer_mobile'],
			  'occasion_type'=>$this->data['occasion_type'],
			  'no_of_seats'=>$this->data['no_of_seats'],				  
			  'booked_date'=>($this->data['date_booking']),
			  'created_date'=>date('c')
			  );					 
			...
			{				
				...
				$res = $command->update('{{table_booking_exception}}' , $params , 
				'id=:id' , array(':id'=> addslashes($this->data['id'])));				
				if ($res)
				{
					$this->code=1;
	                $this->msg=Yii::t("default",'Exception updated.');  
				} else $this->msg=Yii::t("default","ERROR: cannot update");
			} else {		

				if ($res=$command->insert('{{table_booking_exception}}',$params))
				{
					$this->details=Yii::app()->db->getLastInsertID();	                
	                $this->code=1;
	                $this->msg=Yii::t("default",'Exception added.');  	                
	            } else $this->msg=Yii::t("default",'ERROR. cannot insert data.');
			}	
	    ...
	    public function AddOnItemList()
	    {
    	    ...
    	    if(!empty($rows['cat_size_item_price']))
    	    {
    	    		$cat_size_item_price  =	json_decode(json_encode(json_decode($rows['cat_size_item_price'])),True);     	    	    		
    	    }
    	    ...
    	    if (is_array($rows) && count($rows)>=1){
    	    	foreach ($rows as $val) {    	 
    	    		...
    	    		$feed_data['aaData'][]=array(
    	    		  $chk,$val['sub_item_name'].$option,$val['item_description'],$val['item_number'],$cat,
	    ...
	    public function addOnItemNew()
	    {
	    	...
	    	if(sizeof($this->data['size'])>0)
	    	{
	    		foreach ($this->data['size'] as $key => $value) 
	    		{
	    			if($value!=0)
	    			{
						$club['size'][] = $this->data['size'][$key];
						if($this->data['add_on_item_number'][$key]!='')
						{
							$club['add_on_item_number'][] = $this->data['add_on_item_number'][$key];	    					
						}	    				
						if($this->data['add_on_item_price'][$key]!='')
						{
							$club['add_on_item_price'][] = $this->data['add_on_item_price'][$key];	    					
						}	    										   				
	    			}
	    		}
	    	}
			...
			/*	if(empty($club))
				{				
						...
						$req=array(
						  'item_number'=>Yii::t("default","Item_number is required"),
						  'price'=>t("Price is required")
						);	
						...
						{					
							// $this->msg=$Validator->getErrorAsHTML();		
							 $this->msg=Yii::t("default",'Please fill the Mandatory Fields.');  	
							 return ;                 
						}	
				}   */
	    	...
	    	$params=array(
			  ...
			  'item_number'=>isset($this->data['item_number'])?$this->data['item_number']:"",
			  'main_item'=>isset($this->data['item'])?$this->data['item']:"",
			  ...
			  'status'=>addslashes($this->data['status']),
			  'cat_size_item_price'=>$club_value,
			  'date_created'=>date('c'),
			  'ip_address'=>$_SERVER['REMOTE_ADDR'],
			  'merchant_id'=>Yii::app()->functions->getMerchantID()
			);					
	    ...
	    {	    	
	    	...
	    	if (!Yii::app()->functions->validateMerchantCanPost($mtid) ){
	    		if (isset($this->data['id']) && is_numeric($this->data['id'])){				
	    		   ...
	    		   $this->msg=Yii::t("default","Sorry! you reach the limit of adding food item. Please upgrade your membership");
	    		   return ;
	    		}
	    	}	    
	    	...
	    	$enable_deals = isset($this->data['merchant_enable_offer'])?$this->data['merchant_enable_offer']:'';

	    /*	if($enable_deals=="yes")
	    	{	    
	    	  	$deals_radio_opn = isset($this->data['deals_radio_opn'])?$this->data['deals_radio_opn']:'';
	    	  	$requirement = '';
	    		if($deals_radio_opn==2)
	    		{
	    			$req = array('discount'=>"Discount is required");
	    		}		
	    		
			    $Validator=new Validator;
				$req=array(
				  'deal_title'=>Yii::t("default","Deal Title is required"),
				  'deal_description'=>t("Deal Desc is required")
				);	
	    	...
	    	$item_numbers = '';
	    	if (isset($this->data['item_numbers']) && count($this->data['item_numbers'])>=1)
	    	{
	    		foreach ($this->data['item_numbers'] as $key1=>$val1) 
	    		{
	    			if (!empty($val1))
	    			{
	    			   $item_numbers[$this->data['size'][$key1]]=$val1;
	    			}
	    		}	    		
	    		$item_number =	'';
	    	}  
	    	...
	    	$params=array(			  
			  ...
			  'item_category_id'=>isset($this->data['item_category_id'])?($this->data['item_category_id']):'',
			  'item_subcategory_id'=>isset($this->data['sub_cat_id'])?json_encode($this->data['sub_cat_id']):"",
			  'item_number'=>$item_number,
			  'item_num_by_size'=>isset($item_numbers)?json_encode($item_numbers):'', 
			  'status'=>$this->data['status'],
			  ...
			  'enable_offer'=>isset($enable_deals)?$enable_deals:''
			);			
	    ...
	    {	  
    	    ...
    	    if (is_array($rows) && count($rows)>=1){
    	    	foreach ($rows as $val) {    	   
    	    		...
    	    		$item_numbers='';
    	    		$item_numbers=isset($val['item_num_by_size'])?(array)json_decode($val['item_num_by_size']):false;
    	    		if ( is_array($item_numbers) && count($item_numbers)>=1){
    	    			$item_number = '';
    	    			foreach ($item_numbers as $key_item_num=>$val_item_num) {    	    		
    	    				$item_number .=  $val_item_num."<br />\n";
    	    			}
    	    		}
	    ...
	    {	    	
	    	...
	    	Yii::app()->functions->updateOption("merchant_terms_conditions",
	    	isset($this->data['merchant_terms_conditions'])?$this->data['merchant_terms_conditions']:''
	    	,$merchant_id);
	    	...
	    	Yii::app()->functions->updateOption("merchant_pickup_estimation",
	    	isset($this->data['merchant_pickup_estimation'])?$this->data['merchant_pickup_estimation']:''
	    	,$merchant_id);
	    	...
	    	Yii::app()->functions->updateOption("primary_ip",
	    	isset($this->data['primary_ip'])?$this->data['primary_ip']:''
	    	,$merchant_id);
	    	
	    	Yii::app()->functions->updateOption("kitchen_ip",
	    	isset($this->data['kitchen_ip'])?$this->data['kitchen_ip']:''
	    	,$merchant_id); 

	    	Yii::app()->functions->updateOption("merchant_slt_terms_type",
	    	isset($this->data['slt_terms_type'])?$this->data['slt_terms_type']:''
	    	,$merchant_id);

	    	if($this->data['slt_terms_type']==1)
	    	{
	    		Yii::app()->functions->updateOption("merchant_terms_conditions",
		    	isset($this->data['merchant_terms_conditions'])?$this->data['merchant_terms_conditions']:''
		    	,$merchant_id);
	    	...
	    	if($this->data['slt_terms_type']==0)
	    	{
		    	Yii::app()->functions->updateOption("merchant_terms_conditions_url",
		    	isset($this->data['merchant_terms_conditions_url'])?$this->data['merchant_terms_conditions_url']:''
		    	,$merchant_id);	    	
	    	}
	    	...
	    	Yii::app()->functions->updateOption("merchant_required_delivery_time",isset($this->data['merchant_required_delivery_time'])?$this->data['merchant_required_delivery_time']:'',$merchant_id);
	    ...
	    public function socialSettings()
	    {
	    	...
	    	Yii::app()->functions->updateOption("instagram_page",
	    	isset($this->data['instagram_page'])?$this->data['instagram_page']:''
	    	,$merchant_id);	
    ...
    if (is_numeric($mt_delivery_miles)){    	
	    ...
	    {	    	
	    	if (isset($this->data['item_id'])){
	    		...
	    		<p class="uk-alert uk-alert-danger"><?php echo Yii::t("default","Sorry! we cannot find what you are looking for.")?></p>
	        <?php }
	    	die();
	    }	 
		
		public function viewSelectsingleDealsItem()
	    {	    	
	    	if (isset($this->data['item_id']))
	    	{
				$item_id = $this->data['item_id'];	    		
	    		$DbExt=new DbExt;
	    		$stmt = "SELECT `item_num_by_size` FROM `mt_item` WHERE `item_id` = ".$this->data['item_id'];
		    	$res  =  $DbExt->rst($stmt);
		    	$size_name = ''; 
		    	$key = '';
		    	$return_value = '';
		    	 
	    		if(!empty($res[0]['item_num_by_size']))
	    		{
	    			foreach ($res as $size_value) 
	    			{ 
	    				$key = key(json_decode($size_value['item_num_by_size'],true));	    				
	    				if($key!='')
	    				{
		    				$stmt1 = "SELECT `size_name` FROM `mt_size` WHERE `size_id` = ".$key ;	    				 
		    				$res1  =  $DbExt->rst($stmt1);
		    				if(isset($res1[0]['size_name']))
		    				{
		    					$size_name = "|".$res1[0]['size_name']; 	
		    				}
		    				$return_value = json_encode(array('item_id'=>$item_id,'size'=>$size_name,'size_id'=>$key));			    				
	    				}
	    			}
	    			echo $return_value;
	    		}
	    ...
	    public function viewSelectDealsItem()
	    {	    	
	    	if (isset($this->data['item_id'])){
	    		require_once 'deals-item.php';
	    	} else { ?>
	    		<p class="uk-alert uk-alert-danger"><?php echo Yii::t("default","Sorry! we cannot find what you are looking for.")?></p>
	        <?php }
	    	die();
	    ...
	    {	    	
	        if (isset($this->data['order_id'])){
				...
				$data = Yii::app()->functions->getOrder2($this->data['order_id']);				 
 				...
 				if(isset($data['status'])&&$data['status']!="initial_order") { $order_status = $data['status']; }	
				$merchant_id  = $data['merchant_id'];
				$json_details = !empty($data['json_details'])?json_decode($data['json_details'],true):false;
				if ( $json_details !=false){
					Yii::app()->functions->displayOrderHTMLTotalBill(array(
					   'merchant_id'=>$data['merchant_id'],
                ...
                <table class="table table-striped order-det-table">
                    <tbody>	                    
                    	<tr>
                            <td>Order status</td>
                            <td class="text-right"><b><?php echo ucfirst($order_status); ?></b></td>
                        </tr>
                        <tr>
                            <td>Customer Name</td>
                        ...
                        <tr>
                            <td>Merchant Name</td>
                            <td class="text-right"><?php echo stripslashes($data['merchant_name']); ?></td>
                        </tr>
                        <tr>
                            <td>Telephone</td>
                        ...
                        <tr>
                            <td>TRN Type</td>
                            <td class="text-right"><?php echo t($data['trans_type']); ?></td>
                        </tr>
                        <tr>
                            <td>Payment Type</td>
                            <td class="text-right"><?php echo $data['payment_type']; ?></td>
                        </tr>
                        ...
                        <tr>
                            <td>TRN Date</td>
                            <td class="text-right"><?php echo Yii::app()->functions->translateDate($trn_date); ?></td>
                        </tr>
                        <?php
                        if ($data['trans_type']=="delivery"){ /*DELIVERY*/
							if (isset($data['delivery_date'])){
                              ...
                              <tr>
                                    <td>Delivery Date</td>
                                    <td class="text-right"><?php echo FunctionsV3::receiptTableRow('Delivery Date', $date ); ?></td>
                               </tr>
						<?php } } 
					    if (isset($data['delivery_time'])){
						  if ( !empty($data['delivery_time'])){ ?>
                              <tr>
                                    <td>Delivery Time</td>
                                    <td class="text-right"><?php echo Yii::app()->functions->timeFormat($data['delivery_time'],true); ?></td>
                               </tr>  
						<?php  }
					    }
					    if (isset($data['delivery_asap'])){
						   if ( !empty($data['delivery_asap'])){ ?> 
                               <tr>
                                    <td>Deliver ASAP</td>
                                    <td class="text-right"><?php echo $data['delivery_asap']==1?t("Yes"):''; ?></td>
                               </tr> 
					    <?php }
					    }
					    if (!empty($data['client_full_address'])){
 					      ...
 					      <tr>
                            <td>Deliver to</td>
                            <td class="text-right"><?php echo $delivery_address; ?></td>
                          </tr> 
					    ...
					    if (!empty($data['location_name1'])){
					      ...
					      <tr>
                            <td>Location Name</td>
                            <td class="text-right"><?php echo $data['location_name']; ?></td>
                          </tr>  
					    <?php if ( !empty($data['contact_phone1'])){
						  $data['contact_phone']=$data['contact_phone1'];
					    } ?> 
                          <tr>
					    ...
					    <?php if ($data['order_change']>=0.1){ ?> 
                          <tr>
                            <td>Change</td>
                            <td class="text-right"><?php echo FunctionsV3::prettyPrice($data['order_change']); ?></td>
                          </tr> 
					    <?php } ?> 
				    <?php } else { ?> 
						<tr>
				    ...
				    <?php if (isset($data['delivery_date'])){ ?>
                          <tr>
                            <td>Pickup Date</td>
                            <td class="text-right"><?php echo $data['delivery_date']; ?></td>
                          </tr> 
				     <?php } 
					    if (isset($data['delivery_time'])){
						  if ( !empty($data['delivery_time'])){ ?>
                          <tr>
                            <td>Pickup Time</td>
                            <td class="text-right"><?php echo $data['delivery_time']; ?></td>
                          </tr>  
				     <?php }
					    } 
	    ...
	    {	    	
	    	if (isset($this->data['tbl_id'])){  
 			    $res = Yii::app()->functions->getAddressBookByID( isset($this->data['tbl_id'])?$this->data['tbl_id']:'' );
 				//print_r($res);	 
			    echo json_encode($res);
	        }
	    	die();
	    }	 
		public function ClientCcDetails()
	    {	    	
	    	if (isset($this->data['tbl_id'])){  
 			    $res = Yii::app()->functions->getCreditCardInfo( isset($this->data['tbl_id'])?$this->data['tbl_id']:'' ); 
 				//print_r($res);	 
			    echo json_encode($res);
	        }
	    	die();
	    ...
	    {	    		    	
	    	...
	    	$MerchantCurrentStatus = FunctionsV3::getMerchantCurrentStatus($this->data['merchant_id']);
	    	if($MerchantCurrentStatus=="Closed")
	    	{
	    		$this->code=2;	    		
	    	    $this->msg= t(" Merchant is Closed ! ");	 	    	     
	    	    return;
	    	}
	    	...
	    	if (isset($this->data['merchant_id'])){	    		
	    		...
	    		if (isset($_SESSION['kr_item'])) {		
		    		if (is_array($_SESSION['kr_item']) && count($_SESSION['kr_item'])>=1){
		    			...
		    			{		
		    				if(isset($temp_item['merchant_id']))
		    				{
...
/*	    	echo "<pre>";
	    	...
	    	$session_array = array();
	    	foreach ($_SESSION['kr_item'] as $session_key => $session_value) 
	    	{
	    	 	if(is_numeric($session_key))
	    	 	{
	    	 		array_push($session_array,$session_key);
	    	 	}
	    	}
	    	 
	    	if(sizeof($session_array)<1)
	    	{
	    ...
	    public function deleteToppingItem()
	    {
	    	/* print_r($this->data['row']);
	    	print_r($this->data['rel']); 
	    	...
	    	$category_list = explode("_",$this->data['rel']);
	    	
	    	foreach($_SESSION['kr_item'] as $items_listing_key => $items_listing)
	    	{
	    		if(is_numeric($items_listing_key))
	    		{			    	 

			    	// echo "[".$category_list[0]."]['sub_item'][".$category_list[1]."][".$category_list[2]."]"."<br />";
	    			if(isset($items_listing['item_id'])&&$items_listing['item_id']==$category_list[0])
	    			{
	    				//echo " category_list[0]  ". $category_list[0]."['sub_item']"."[".$category_list[1]."]"."[".$category_list[2]."]";

	    				unset($_SESSION['kr_item'][$items_listing_key]['sub_item'][$category_list[1]][$category_list[2]]); 
	    			}	    			 
	    		}	    		
	    	}
	    	...
	    	/* exit;
	    	if ( isset($_SESSION['kr_item'][$category_list[0]]['sub_item'][$category_list[1]][$category_list[2]]))
	    	{
	    			    		
	    		//if (is_numeric($row_api_id)){
	    		if (isset($_SESSION['kr_item'][$category_list[0]]['sub_item'][$category_list[1]][$category_list[2]]['row_api_id'])){
	    			$row_api_id=$_SESSION['kr_item'][$category_list[0]]['sub_item'][$category_list[1]][$category_list[2]]['row_api_id'];	 
	    			$ApiFunctions=new ApiFunctions;
	    			$ApiFunctions->deleteItemFromCart($row_api_id);
	    		}	    	
	    		
	      	   unset($_SESSION['kr_item'][$category_list[0]]['sub_item'][$category_list[1]][$category_list[2]]);
	    	} */
	    }
	   ...
	   public function setDeliveryOptions()
	    { 
	    	...
	    	{	
	    		$this->msg=t("Please select the Delivery Time ");
		       	return ;	      
	    	}
	       	  	...
	       	  	date_default_timezone_set("Europe/Jersey");
	       	  	 ...
	       	  	 if (is_array($time_diff) && count($time_diff)>=1){
	       	  	    // if ( $time_diff['hours']>0){	       	  	     	
		       	  	     $this->msg=t("Sorry! you have selected an Invalid Time ");
		       	  	     return ;	       	  	     	
	       	  	    // }	       	  	
	       	  	 }	       	  
	       	  }	       
		   ...
		   {		   		 
				...
				$this->msg=t("Sorry! we are closed on")." ".$date_close;
				$this->msg.="<br/>";
				$this->msg.=t("Please check merchant opening hours");
			    return ;
			}			 

			$error_reason = Yii::app()->session['error_reason'];			
			if(isset($error_reason)&&!empty($error_reason))
			{
				$this->msg=t("Sorry ")." ".$date_close;
				$this->msg.="<br/>";
				$this->msg.=t($error_reason);
				$this->msg.="<br/>";
				$this->msg.=t("Please check merchant opening hours");
				unset(Yii::app()->session['error_reason']);
			    return ;	
			}
	       ...
	       $_SESSION['kr_delivery_options']['order_type']=$this->data['order_type'];

			//if (Yii::app()->functions->isClientLogin()){ 			
			/*if (!Yii::app()->functions->isClientLogin()){	       
			   $this->code=2;$this->msg=Yii::t("default","Not Logged");	
			   return ;
			}*/	 
        ...
        { 
           ...
           if ( isset($this->data['delivery_time'])){
                 if(!empty($this->data['delivery_time'])){
                      ...
                      if (is_array($time_diff) && count($time_diff)>=1){
                          if ( $time_diff['hours']>0){                              
                              $this->msg=t("Sorry! you have selected time that already past");
                              return ;                              
                          }                     
                      }                 
                 }           
           }                    
           $time = strtotime($this->data['delivery_time']);           
        /*   $endTime = date("H:i", strtotime('+50 minutes', $time));                      
           $_SESSION['kr_delivery_options']['delivery_type']=$this->data['delivery_type'];
           $_SESSION['kr_delivery_options']['delivery_date']=$this->data['delivery_date'];
           //$_SESSION['kr_delivery_options']['delivery_time']=$this->data['delivery_time'];
           $_SESSION['kr_delivery_options']['delivery_time']=$endTime;
           $_SESSION['kr_delivery_options']['delivery_asap']=$this->data['delivery_asap']=="undefined"?"":1;                        commented on 16-05-217         
            ...
            if (!array_key_exists($full_booking_day,$business_hours)){                
                ...
                if (!empty($booking_time)){                    
                    ...
                    {                            
                        ...
                        {        // for $selected_date[0] starting date and time   
                            ...
                            {                                    
                                ...
                                $t2=trim($selected_date[1]);                                
                                if ( $this->checkBetweenTime($booking_time,$t1,$t2))
                                {        // for $selected_date[1] ending date and time
                                ...
                                /*                                   
                                    $break_loop = false ;    
                                    $next_day   = ''; 
                                    foreach($business_hours as $key => $opening_days)
                                    {
                                         if($break_loop)
                                         {
                                             $next_day  = $key; 
                                    ...
                                    $selected_date=$business_hours[$next_day]; 
                                    print_r($selected_date);        exit;
			                        //dump($selected_date);
			                        $temp_selected=explode(",",$selected_date);    
			                        //dump($temp_selected);    
			                        ...
			                        echo $t1 ; 
			                        exit;
                                    $delivery_date = date('Y-m-d', strtotime($this->data['delivery_date'] . ' +1 day'));
    ...
    {   
		...
		echo  $current_time . "    " .$opening_time ."    " . $closing_time ;
	    ...
	    {	    	
	    	...
	    	if ( FunctionsK::emailBlockedCheck($this->data['email_address'])){
	    		$this->msg=t("Sorry! your email address is blocked by website admin");
	    		return ;
	    	}	    
	    	
	    	if ( FunctionsK::mobileBlockedCheck($this->data['contact_phone'])){
	    		$this->msg=t("Sorry! your mobile number is blocked by website admin");
	    		return ;
	    	}	    	
	        ...
	        if ( !$res=Yii::app()->functions->isClientExist($this->data['email_address']) ){
		        if ( $functionk->CheckCustomerMobile($this->data['contact_phone'])){
		        	$this->msg=t("Sorry! your mobile number is already exist in our records");
		        	return ;
		        }	  
	        }
	    	...
	    	if ( !$res=Yii::app()->functions->isClientExist($this->data['email_address']) ){
	    	   ...
	    	   $this->msg=Yii::t("default","Sorry! your email address already exist in our records.");
	    	}
	    }		    	 
	    ...
	    {	  
	    	...
	    	if (isset($this->data['client_order_sms_code'])){
	    		...
	    		if (!empty($this->data['client_order_sms_code'])){
	    			...
	    			$this->data['client_order_session'])){	    				
	    				$this->msg=t("Sorry! you have input invalid order sms code");
	    				return ;
	    			}	    		
	    		}
	    	}	    	 
	    	...
	    	if (isset($this->data['parish']))
	    	{
	    		if($this->data['parish']==0)
	    		{
	    			$this->msg=t("Please select a parish");
	    			return ;
	    		}
	    	}
		    	...
		    	if (!$functionsk->reCheckDelivery($this->data,$mtid)){
		    		...
		    		$this->msg=t("Sorry! this merchant delivers only with in ").$mt_delivery_miles."$unit";
		    		return ;
		    	}*/		    	
	    	...
	    	{	    		
				...
				$_SESSION['kr_client_details'] = $this->data;				    

				$this->data['email_address'] = $this->data['email_address1'];
				$this->data['password'] = $this->data['password1'];
			    ...
			    if($this->data['save_as_member']==2)
			    {
			    		...
			    		if ($res_check=Yii::app()->functions->isClientExist($this->data['email_address']) )
			    		{	
			    			$this->msg=t("Sorry you're a registered user  ");
				    		return ;
			    		}	
		    		...
		    		// } else $Validator->msg[]=t("Sorry! your email address already exist in our records.");
	    	...
	    	if (is_array($order_item) && count($order_item)>=1){	
	    		...
	    		switch ($this->data['payment_opt'])
	    		{
                    ...
                    case "cpy":
	    				if ( Yii::app()->functions->isMerchantCommission($this->data['merchant_id'])){
	    					...
	    					$card_fee=Yii::app()->functions->getOption('merchant_citypay_fee',
	    					$this->data['merchant_id']);
	    				}	    		    					    				
						break;    
					case "cpn":	
						if ( Yii::app()->functions->isMerchantCommission($this->data['merchant_id']))
						{
	    					$card_fee=Yii::app()->functions->getOptionAdmin('merchant_chip_pin_fee');
						} else 
						{
	    					$card_fee=Yii::app()->functions->getOption('merchant_chip_pin_fee',$this->data['merchant_id']);
	    				}	    		    					    				
						break;    
	    			default:	    				
	    				break;		
	    		...
	    		//  Deals Discount part adding discount amount  21-08-2017

	    		/*	if($this->data['deals_discount']!=''&&isset($_SESSION['kr_item'][0]['discount']))
	    			{
	    					$_SESSION['kr_item'][0]['discount'] += 	$this->data['deals_discount'];
	    			}  */

	    	// 	Yii::app()->functions->displayOrderHTML($this->data,$_SESSION['kr_item']);
	    		Yii::app()->functions->displayOrderHTMLOriginal($this->data,$_SESSION['kr_item']);	
	    		...
	    		if ( Yii::app()->functions->code==1){
	    			...
	    			// print_r($_SESSION['kr_item']);	    			 
	    			$payment_type = '';
	    			if($this->data['payment_opt']=="cod")
	    			{
	    				$payment_type = "cash";	
	    			}
	    			
	    			if($this->data['payment_opt']=="cpy")
	    			{
	    				if($this->data['is_guest_checkout']==2)
	    				{
	    					$_SESSION['guest_checkout_cpy'] = "true";
	    				}
	    				$payment_type = "citypay";	
	    			}
 					
 					if($this->data['payment_opt']=="pyp")
	    			{
	    				$payment_type = "paypal";	
	    			}    			

					if($this->data['payment_opt']=="cpn")
	    			{
	    				$payment_type = "chippin";	
	    			}    			
	    			...
	    			*/
	    			...
	    			if (is_array($raw) && count($raw)>=1){	    				
	    				$params=array(
	    				  ...
	    				  'free_details'=>isset($_SESSION['kr_item']['free_items'])?json_encode($_SESSION['kr_item']['free_items']):'',			
	    				  'discount_details'=>isset($_SESSION['kr_item']['total_discount'])?json_encode($_SESSION['kr_item']['total_discount']):'',			
	    				  ...
	    				  'bill_total'=>isset($raw['bill_total'])?$raw['bill_total']:0,
...
if ($this->data['payment_opt']=="cod" || $this->data['payment_opt']=="pyr" || $this->data['payment_opt']=="ccr" || $this->data['payment_opt']=="ocr" ){
		    			...
		    			if($this->data['deals_discount']!='')
		    			{		    					
								$this->data['deals_discount']   = str_replace('£ ','',$this->data['deals_discount']);
		    					$params['deals_discount_amt']   = 	$this->data['deals_discount'];
		    			}  	    					 					    				
	    				/*PROMO*/	    				
...
$params['cart_tip_value']=isset($this->data['cart_tip_value'])?$this->data['cart_tip_value']:'';                        			            
		    		        ...
		    		        if (isset($this->data['map_address_toogle'])){
		    		        	if ( $this->data['map_address_toogle']==2){
		    		        	    ...
		    		        	    if ($geo_res){
		    		        			...
		    		        			$this->msg=t("Sorry! something wrong when geocoding your address");
		    		        			return false;
		    		        		}		  
		    		        	}	    		        
		    		        }	    		
		    			...
		    			/*	$responding_json['id']			 = $order_id;
		    				...
		    				$stmt1 = " SELECT * FROM `mt_order_delivery_address` WHERE `order_id` = 113 ";
		    				$res1=$DbExt->rst($stmt1);
		    				...
		    				/* if($this->data['is_guest_checkout'])
		    				{
		    					...
		    					{
		    						$guest_client_id = Yii::app()->functions->getClientId();	
		    					}
		    				} */
		    				...
		    				{			
			    				$params_address=array(
			    				  'order_id'=>$order_id,
			    				  'client_id'=>isset($this->data['is_guest_checkout'])?0:Yii::app()->functions->getClientId(),
			    				...
			    				$DbExt=new DbExt;
			    		  		...
			    		  		{
			    		  					$responding_json['first_name'] 	= $this->data['first_name'];
					    					$responding_json['last_name'] 	= $this->data['last_name'];		
			    		  		}
			    				...
			    				foreach($res1 as $delivery_details)
			    				{
			    					...
			    					$total_amount  = 0 ; 
			    				//print_r($responding_json); 
			    				
			    				$cart_value = ''; 
			    				foreach($order_item as $key=>$order_details)
			    				{
			    					if(is_int($key))
			    					{	
				    					...
				    					{
				    						foreach($order_details['sub_item'] as $sub_item_key=>$sub_item_list)
				    						{				    							
				    							$sub_itm_id = '';
			    								$sub_itm_pr = '';
			    								$sub_itm_tp = '';
				    							...
				    							{				    								 
				    								...
				    								$sub_itm_pr = $sub_itm_details[1];
				    								$sub_itm_tp = $sub_itm_details[2];
				    ...
				    $sub_item_details[] = array('subcat_id'=>$sub_item_key,'value'=>$sub_itm_id."|".$sub_itm_pr."|".$sub_itm_tp,"qty"=>"itemqty");
				    						}
				    					}
				    					...
				    					$cart_value[] = array(
											  ...
											  "price"=>str_replace('"','__',$order_details['price']),
											  "sub_item"=>$sub_item_details,
											  "cooking_ref"=>isset($order_details['cooking_ref'])?$order_details['cooking_ref']:'',
											  "ingredients"=>isset($order_details['ingredients'])?$order_details['ingredients']:'',
											  'order_notes'=>isset($order_details['notes'])?$order_details['notes']:'',
			    				...
			    				if(sizeof($cart_value)>0)
			    				{			    	
			    					/* echo "<pre>";
			    					print_r($cart_value);				 
			    					echo "</pre>";
			    					$update_cart['mobile_cart_details'] = json_encode($cart_value,true);			    					 
			    					echo $order_id ;
			    					print_r($update_cart);
			    					exit;	 */
			    					...
			    					$this->updateData("{{order}}",$update_cart,'order_id',$order_id);			    				 
			    				}
			    				...
			    				$responding_json['total'] = number_format($params['total_w_tax'], 2, '.', '');			    
			    				$responding_json['discount'] = isset($params['discounted_amount'])?round($params['discounted_amount'], 2):0;
			    				$responding_json['payment'] = array('type'=>$params['payment_type'],'amount'=>number_format($params['total_w_tax'], 2, '.', ''));		    				
			    				
			    					 
			    				if(Yii::app()->functions->getClientId()!='')
			    				{
			    					/** quick update mobile*/
			    					...
			    					if(isset($this->data['contact_phone']))
			    					{
			    						...
			    						{
											$params_mobile=array('contact_phone'=>$this->data['contact_phone']);
			    						}
			    					}
			    					...
			    					if(isset($this->data['location_name']))
			    					{
			    						...
			    						{
											$params_mobile=array('location_name'=>$this->data['location_name']);
			    						}
			    					}
		    		...
		    		//		}
						...
						/*	if(Yii::app()->functions->getClientId()!='')	
							{								  */
								if(isset($this->data['save_as_member'])&&$this->data['save_as_member']==2)
			    				{
			    						$this->data['saved_address']='2';
			    				}
			    				...
			    				if (!isset($this->data['saved_address']))
			    				{
			    				 ...
			    				 {		
			    					...
			    					if ($this->data['address_book_id']=='')
						            {					            	
						            	$sql_up= "UPDATE `mt_address_book` SET `as_default`= 1 WHERE `client_id` = ".Yii::app()->functions->getClientId();
							     		$this->qry($sql_up);
						            	$params_i=array(
						            	  'client_id'=>Yii::app()->functions->getClientId(),
						            	  'street'=>$this->data['street'],
						            ...
						            else
						            {
						            	...
						            	$sql_up1 = "UPDATE `mt_address_book` SET `as_default`= 1 WHERE `client_id` = ".Yii::app()->functions->getClientId();
						            	if($this->qry($sql_up1))
						            	{
						            		$sql_up2 ="UPDATE `mt_address_book` SET `as_default`= 2 WHERE id = ".$this->data['address_book_id'];
							     			$this->qry($sql_up2);
						            	}					            	
						            }
			    				}					            
						//	}	    				
			    				if((Yii::app()->functions->getClientId()!='')&&($this->data['delivery_type']=="delivery"))
			    				{

			    				/* $signup_client_stmt = 'SELECT * FROM  `mt_address_book` WHERE  `client_id` = '.Yii::app()->functions->getClientId();		    $DbExt=new DbExt;				
						    		if(!$signup_client_res = $DbExt->rst($signup_client_stmt))
						    		{ */

						    		$client_address_stmt =	'SELECT * FROM `mt_address_book` WHERE `client_id` = '.Yii::app()->functions->getClientId();
					    			if($client_address_res = $DbExt->rst($client_address_stmt))
					    			{
					    				...
					    				foreach ($client_address_res as $client_address_value) 
					    				{
					    					...
					    					$address_found = false; 
/* echo trim($this->data['location_name'])."  ".trim($client_address_value['location_name'])." <br /> ";			
echo trim($this->data['street'])." ".trim($client_address_value['street'])." <br /> ";		    					
echo strcmp(trim($this->data['location_name']),trim($client_address_value['location_name']))."<br />";
echo strcmp(trim($this->data['street']),trim($client_address_value['street']))."<br />";
*/
if(strcmp(trim($this->data['location_name']),trim($client_address_value['location_name']))==0&&strcmp(trim($this->data['street']),trim($client_address_value['street']))==0)	
					    					 {					    					 	 
					 	$sql_up5 = "UPDATE `mt_address_book` SET `as_default`= 1 WHERE `client_id` = ".Yii::app()->functions->getClientId();
							            		$this->qry($sql_up5);
					    					 	$address_found = true; 					     
					    $sql_up3 = "UPDATE `mt_address_book` SET `as_default`= 2 WHERE `id` = ".$client_address_value['id'];
					    				...
					    				if(!$address_found)
					    				{
					    					$sql_up3 = "UPDATE `mt_address_book` SET `as_default`= 1 WHERE `client_id` = ".Yii::app()->functions->getClientId();
							            	$this->qry($sql_up3);
							    				$params_i=array(
								            	  'client_id'=>Yii::app()->functions->getClientId(),
								            	  'street'=>$this->data['street'],
		    			...
		    			/*	$external_json_stmt = 'SELECT * FROM mt_external_json WHERE `merchant_list` LIKE \'%"'.$mtid.'"%\'';		    				
		    				...
		    				{		    					
		    					...
		    					//	$curl_response = $this->getData($url,$responding_json);
		    						$merchant_json_list[] = $responding_json;
		    					}		    					
		    					
		    				}   
		    				Yii::app()->session['merchant_json_list'] = $merchant_json_list; 	*/	    			

		    				Yii::app()->session['merchant_json_list'] = $responding_json;
	    	...
	    	} else $this->msg=Yii::t("default","Sorry! your order is empty");	    
	    }
    ...
    return $data;
	    ...
	    {	    	
	    	...
	    	} else $this->msg=Yii::t("default","Sorry! you need to login before you can make a rating.");	    	
	    }
	    ...
	    {	    		    		    	
	    	...
	    	if ( Yii::app()->functions->isClientLogin()){
		    	...
		    	if ( Yii::app()->functions->getOptionAdmin('website_reviews_actual_purchase')=="yes"){
		    	    ...
		    	    if (!$functionk->canReviewBasedOnOrder($client_id,$this->data['merchant-id'])){
		    		   $this->msg=t("Sorry! you can make one review per order");
		    	       return ;
		    	    }	  		   
	    	...
	    	} else $this->msg=Yii::t("default","Sorry! you need to login to write a review.");
	    }
	    	...
	    	if ( $res=Yii::app()->functions->getReviewsList($this->data['merchant_id']) ){	    		
	    		...
	    		<li class="media">
                    <a class="pull-left" href="javascript:;">
                   	<!-- <img src="<?php echo FunctionsV3::getAvatar($val['client_id']);?>" alt="" class="img-circle" width="50" height="50"> -->
                    </a>
                    <div class="media-body">  
                        <div class="feedback-info">
                            <div class="feedback-author">
                                <a href="javascript:;"><?php echo $val['client_name']?></a>
                            </div>
                            <span class="feedback-time"><?php echo $pretyy_date;?></span>
                        </div>
                        <div class="rating-stars" data-score="<?php echo $val['rating']?>"></div>
                        <p><?php echo nl2br($val['review'])?> </p>
                        <div class="review-edit-btns">
						<?php if ( $val['client_id']==$client_id ):?>
                            <a href="javascript:;" data-id="<?php echo $val['id']?>" class="delete-review btn btn-sm btn-danger  pull-right">
                                <?php echo t("Delete")?>
                            </a>
                            <a href="javascript:;" data-id="<?php echo $val['id']?>" class="edit-review btn btn-sm btn-success  pull-right">
                           		<?php echo t("Edit")?>
                            </a> 
                        <?php endif;?>
                        </div>
	            ...
	            $this->msg="OK";	    		
	            $this->details=$html;
	    	} else $this->msg=Yii::t("default",'<div class="text-center alert alert-danger mb-0">No reviews yet.</div>');	
	    }
	    
		    public function saveMerchantcitypaySettings()
		{
			 $merchant_id = Yii::app()->functions->getMerchantID();   			 
			                                                       
     		Yii::app()->functions->updateOptionMerchant("merchant_enabled_citypay",$merchant_id,
	    	isset($this->data['merchant_enabled_citypay'])?$this->data['merchant_enabled_citypay']:'');
		                                  
	    	Yii::app()->functions->updateOptionMerchant("merchant_citypay_mode",$merchant_id,
	    	isset($this->data['merchant_citypay_mode'])?$this->data['merchant_citypay_mode']:'' );
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_sanbox_citypay_user",$merchant_id,
	    	isset($this->data['merchant_sanbox_citypay_user'])?$this->data['merchant_sanbox_citypay_user']:'' );
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_sanbox_citypay_pass",$merchant_id,
	    	isset($this->data['merchant_sanbox_citypay_pass'])?$this->data['merchant_sanbox_citypay_pass']:'' );                                                        
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_live_citypay_user",$merchant_id,
	    	isset($this->data['merchant_live_citypay_user'])?$this->data['merchant_live_citypay_user']:'' );
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_live_citypay_pass",$merchant_id,
	    	isset($this->data['merchant_live_citypay_pass'])?$this->data['merchant_live_citypay_pass']:'' );
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_live_citypay_signature",$merchant_id,
	    	isset($this->data['merchant_live_citypay_signature'])?$this->data['merchant_live_citypay_signature']:'');
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_citypay_fee",$merchant_id,
	    	isset($this->data['merchant_citypay_fee'])?$this->data['merchant_citypay_fee']:'');
	    	
	    	Yii::app()->functions->updateOptionMerchant("mt_citypay_mobile_enabled",$merchant_id,
	    	isset($this->data['mt_citypay_mobile_enabled'])?$this->data['mt_citypay_mobile_enabled']:'');
	    	
	    	Yii::app()->functions->updateOptionMerchant("mt_citypay_mobile_mode",$merchant_id,
	    	isset($this->data['mt_citypay_mobile_mode'])?$this->data['mt_citypay_mobile_mode']:'');
	    	
	    	/*
	    	Yii::app()->functions->updateOptionMerchant("mt_citypay_mobile_clientid",$merchant_id,
	    	isset($this->data['mt_citypay_mobile_clientid'])?$this->data['mt_citypay_mobile_clientid']:''); */

	    	Yii::app()->functions->updateOptionMerchant("merchant_mobile_sanbox_citypay_pass",$merchant_id,
	    	isset($this->data['merchant_mobile_sanbox_citypay_pass'])?$this->data['merchant_mobile_sanbox_citypay_pass']:'' );
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_mobile_sandbox_citypay_user",$merchant_id,
	    	isset($this->data['merchant_mobile_sandbox_citypay_user'])?$this->data['merchant_mobile_sandbox_citypay_user']:'' );                                                        
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_mobile_live_citypay_user",$merchant_id,
	    	isset($this->data['merchant_mobile_live_citypay_user'])?$this->data['merchant_mobile_live_citypay_user']:'' );
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_mobile_live_citypay_pass",$merchant_id,
	    	isset($this->data['merchant_mobile_live_citypay_pass'])?$this->data['merchant_mobile_live_citypay_pass']:'' );
		    ...
		    public function saveMerchantchip_pinSettings()
		{
			 $merchant_id = Yii::app()->functions->getMerchantID();   			 
			                                                       
     		Yii::app()->functions->updateOptionMerchant("merchant_enabled_chip_pin",$merchant_id,
	    	isset($this->data['merchant_enabled_chip_pin'])?$this->data['merchant_enabled_chip_pin']:'');
		                                  
	    	Yii::app()->functions->updateOptionMerchant("merchant_chip_pin_mode",$merchant_id,
	    	isset($this->data['merchant_chip_pin_mode'])?$this->data['merchant_chip_pin_mode']:'' );
	    	...
	    	Yii::app()->functions->updateOptionMerchant("merchant_sanbox_chip_pin_user",$merchant_id,
	    	isset($this->data['merchant_sanbox_chip_pin_user'])?$this->data['merchant_sanbox_chip_pin_user']:'' );
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_sanbox_chip_pin_pass",$merchant_id,
	    	isset($this->data['merchant_sanbox_chip_pin_pass'])?$this->data['merchant_sanbox_chip_pin_pass']:'' );


	    	Yii::app()->functions->updateOptionMerchant("merchant_sanbox_chip_pin_user_id",$merchant_id,
	    	isset($this->data['merchant_sanbox_chip_pin_user_id'])?$this->data['merchant_sanbox_chip_pin_user_id']:'' );

	    	Yii::app()->functions->updateOptionMerchant("merchant_sanbox_chip_pin_password",$merchant_id,
	    	isset($this->data['merchant_sanbox_chip_pin_password'])?$this->data['merchant_sanbox_chip_pin_password']:'' );

	    	Yii::app()->functions->updateOptionMerchant("merchant_sanbox_chip_pin_client_id",$merchant_id,
	    	isset($this->data['merchant_sanbox_chip_pin_client_id'])?$this->data['merchant_sanbox_chip_pin_client_id']:'' );	    	

                                             
             // Live            
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_live_chip_pin_user",$merchant_id,
	    	isset($this->data['merchant_live_chip_pin_user'])?$this->data['merchant_live_chip_pin_user']:'' );
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_live_chip_pin_pass",$merchant_id,
	    	isset($this->data['merchant_live_chip_pin_pass'])?$this->data['merchant_live_chip_pin_pass']:'' );

	    	Yii::app()->functions->updateOptionMerchant("merchant_live_chip_pin_user_id",$merchant_id,
	    	isset($this->data['merchant_live_chip_pin_user_id'])?$this->data['merchant_live_chip_pin_user_id']:'' );

	    	Yii::app()->functions->updateOptionMerchant("merchant_live_chip_pin_password",$merchant_id,
	    	isset($this->data['merchant_live_chip_pin_password'])?$this->data['merchant_live_chip_pin_password']:'' );

	    	Yii::app()->functions->updateOptionMerchant("merchant_live_chip_pin_client_id",$merchant_id,
	    	isset($this->data['merchant_live_chip_pin_client_id'])?$this->data['merchant_live_chip_pin_client_id']:'' );




	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_live_chip_pin_signature",$merchant_id,
	    	isset($this->data['merchant_live_chip_pin_signature'])?$this->data['merchant_live_chip_pin_signature']:'');
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_chip_pin_fee",$merchant_id,
	    	isset($this->data['merchant_chip_pin_fee'])?$this->data['merchant_chip_pin_fee']:'');
	    	
	    	Yii::app()->functions->updateOptionMerchant("mt_chip_pin_mobile_enabled",$merchant_id,
	    	isset($this->data['mt_chip_pin_mobile_enabled'])?$this->data['mt_chip_pin_mobile_enabled']:'');
	    	
	    	Yii::app()->functions->updateOptionMerchant("mt_chip_pin_mobile_mode",$merchant_id,
	    	isset($this->data['mt_chip_pin_mobile_mode'])?$this->data['mt_chip_pin_mobile_mode']:'');
	    	
	    	/*
	    	Yii::app()->functions->updateOptionMerchant("mt_chip_pin_mobile_clientid",$merchant_id,
	    	isset($this->data['mt_chip_pin_mobile_clientid'])?$this->data['mt_chip_pin_mobile_clientid']:''); */



	    		// Mobile Sandbox 

	    	Yii::app()->functions->updateOptionMerchant("merchant_mobile_sanbox_chip_pin_pass",$merchant_id,
	    	isset($this->data['merchant_mobile_sanbox_chip_pin_pass'])?$this->data['merchant_mobile_sanbox_chip_pin_pass']:'' );
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_mobile_sandbox_chip_pin_user",$merchant_id,
	    	isset($this->data['merchant_mobile_sandbox_chip_pin_user'])?$this->data['merchant_mobile_sandbox_chip_pin_user']:'' );

    	 	Yii::app()->functions->updateOptionMerchant("merchant_mobile_sanbox_chip_pin_user_id",$merchant_id,
	    	isset($this->data['merchant_mobile_sanbox_chip_pin_user_id'])?$this->data['merchant_mobile_sanbox_chip_pin_user_id']:'' );

	    	Yii::app()->functions->updateOptionMerchant("merchant_mobile_sanbox_chip_pin_password",$merchant_id,
	    	isset($this->data['merchant_mobile_sanbox_chip_pin_password'])?$this->data['merchant_mobile_sanbox_chip_pin_password']:'' );

	    	Yii::app()->functions->updateOptionMerchant("merchant_mobile_sanbox_chip_pin_client_id",$merchant_id,
	    	isset($this->data['merchant_mobile_sanbox_chip_pin_client_id'])?$this->data['merchant_mobile_sanbox_chip_pin_client_id']:'' );	


	    	// Mobile Live 
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_mobile_live_chip_pin_user",$merchant_id,
	    	isset($this->data['merchant_mobile_live_chip_pin_user'])?$this->data['merchant_mobile_live_chip_pin_user']:'' );
	    	
	    	Yii::app()->functions->updateOptionMerchant("merchant_mobile_live_chip_pin_pass",$merchant_id,
	    	isset($this->data['merchant_mobile_live_chip_pin_pass'])?$this->data['merchant_mobile_live_chip_pin_pass']:'' );


	    	Yii::app()->functions->updateOptionMerchant("merchant_mobile_live_chip_pin_user_id",$merchant_id,
	    	isset($this->data['merchant_mobile_live_chip_pin_user_id'])?$this->data['merchant_mobile_live_chip_pin_user_id']:'' );

	    	Yii::app()->functions->updateOptionMerchant("merchant_mobile_live_chip_pin_password",$merchant_id,
	    	isset($this->data['merchant_mobile_live_chip_pin_password'])?$this->data['merchant_mobile_live_chip_pin_password']:'' );

	    	Yii::app()->functions->updateOptionMerchant("merchant_mobile_live_chip_pin_client_id",$merchant_id,
	    	isset($this->data['merchant_mobile_live_chip_pin_client_id'])?$this->data['merchant_mobile_live_chip_pin_client_id']:'' );
            ...
            public function saveAdminCitypaySettings()
		{                                                                    
                
			Yii::app()->functions->updateOptionAdmin("admin_enabled_citypay",
	    	isset($this->data['admin_enabled_citypay'])?$this->data['admin_enabled_citypay']:'');
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_citypay_mode",
	    	isset($this->data['admin_citypay_mode'])?$this->data['admin_citypay_mode']:'' );
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_sanbox_citypay_user",
	    	isset($this->data['admin_sanbox_citypay_user'])?$this->data['admin_sanbox_citypay_user']:'' );
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_sanbox_citypay_pass",
	    	isset($this->data['admin_sanbox_citypay_pass'])?$this->data['admin_sanbox_citypay_pass']:'' );
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_sanbox_citypay_signature",
	    	isset($this->data['admin_sanbox_citypay_signature'])?$this->data['admin_sanbox_citypay_signature']:'' );
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_live_citypay_user",
	    	isset($this->data['admin_live_citypay_user'])?$this->data['admin_live_citypay_user']:'' );
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_live_citypay_pass",
	    	isset($this->data['admin_live_citypay_pass'])?$this->data['admin_live_citypay_pass']:'' );
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_live_citypay_signature",
	    	isset($this->data['admin_live_citypay_signature'])?$this->data['admin_live_citypay_signature']:'');
	    	

	    	Yii::app()->functions->updateOptionAdmin("admin_mob_sanbox_citypay_user",
	    	isset($this->data['admin_mob_sanbox_citypay_user'])?$this->data['admin_mob_sanbox_citypay_user']:'' );
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_mob_sanbox_citypay_pass",
	    	isset($this->data['admin_mob_sanbox_citypay_pass'])?$this->data['admin_mob_sanbox_citypay_pass']:'' );
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_mob_live_citypay_user",
	    	isset($this->data['admin_mob_live_citypay_user'])?$this->data['admin_mob_live_citypay_user']:'' );
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_mob_live_citypay_pass",
	    	isset($this->data['admin_mob_live_citypay_pass'])?$this->data['admin_mob_live_citypay_pass']:'');



	    	Yii::app()->functions->updateOptionAdmin("admin_citypay_fee",
	    	isset($this->data['admin_citypay_fee'])?$this->data['admin_citypay_fee']:'');
	    	
	    	Yii::app()->functions->updateOptionAdmin("adm_citypay_mobile_enabled",
	    	isset($this->data['adm_citypay_mobile_enabled'])?$this->data['adm_citypay_mobile_enabled']:'');
	    	
	    	Yii::app()->functions->updateOptionAdmin("adm_citypay_mobile_mode",
	    	isset($this->data['adm_citypay_mobile_mode'])?$this->data['adm_citypay_mobile_mode']:'');
	    	
	    	Yii::app()->functions->updateOptionAdmin("adm_citypay_mobile_clientid",
	    	isset($this->data['adm_citypay_mobile_clientid'])?$this->data['adm_citypay_mobile_clientid']:'');
	    	
	    	$this->code=1;
	    	$this->msg=Yii::t("default","Setting saved");
		}	    
               
            public function saveAdminChippinSettings()
		{                                                                    
                
			Yii::app()->functions->updateOptionAdmin("admin_enabled_chip_pin",
	    	isset($this->data['admin_enabled_chip_pin'])?$this->data['admin_enabled_chip_pin']:'');
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_chip_pin_mode",
	    	isset($this->data['admin_chip_pin_mode'])?$this->data['admin_chip_pin_mode']:'' );


	    	// Sandbox 
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_sanbox_chip_pin_user",
	    	isset($this->data['admin_sanbox_chip_pin_user'])?$this->data['admin_sanbox_chip_pin_user']:'' );
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_sanbox_chip_pin_pass",
	    	isset($this->data['admin_sanbox_chip_pin_pass'])?$this->data['admin_sanbox_chip_pin_pass']:'' );

	    	   	Yii::app()->functions->updateOptionAdmin("admin_sanbox_chip_pin_user_id",$admin_id,
	    	isset($this->data['admin_sanbox_chip_pin_user_id'])?$this->data['admin_sanbox_chip_pin_user_id']:'' );

	    	Yii::app()->functions->updateOptionAdmin("admin_sanbox_chip_pin_password",$admin_id,
	    	isset($this->data['admin_sanbox_chip_pin_password'])?$this->data['admin_sanbox_chip_pin_password']:'' );

	    	Yii::app()->functions->updateOptionAdmin("admin_sanbox_chip_pin_client_id",$admin_id,
	    	isset($this->data['admin_sanbox_chip_pin_client_id'])?$this->data['admin_sanbox_chip_pin_client_id']:'' );	    	
	    	...
	    	Yii::app()->functions->updateOptionAdmin("admin_sanbox_chip_pin_signature",
	    	isset($this->data['admin_sanbox_chip_pin_signature'])?$this->data['admin_sanbox_chip_pin_signature']:'' );
	    	


				// Live 

	    	Yii::app()->functions->updateOptionAdmin("admin_live_chip_pin_user",
	    	isset($this->data['admin_live_chip_pin_user'])?$this->data['admin_live_chip_pin_user']:'' );
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_live_chip_pin_pass",
	    	isset($this->data['admin_live_chip_pin_pass'])?$this->data['admin_live_chip_pin_pass']:'' );


	    	Yii::app()->functions->updateOptionAdmin("admin_live_chip_pin_user_id",$admin_id,
	    	isset($this->data['admin_live_chip_pin_user_id'])?$this->data['admin_live_chip_pin_user_id']:'' );

	    	Yii::app()->functions->updateOptionAdmin("admin_sanbox_chip_pin_password",$admin_id,
	    	isset($this->data['admin_live_chip_pin_password'])?$this->data['admin_live_chip_pin_password']:'' );

	    	Yii::app()->functions->updateOptionAdmin("admin_live_chip_pin_client_id",$admin_id,
	    	isset($this->data['admin_live_chip_pin_client_id'])?$this->data['admin_live_chip_pin_client_id']:'' );	 




	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_live_chip_pin_signature",
	    	isset($this->data['admin_live_chip_pin_signature'])?$this->data['admin_live_chip_pin_signature']:'');
	    	...
	    	Yii::app()->functions->updateOptionAdmin("admin_mob_sanbox_chip_pin_user",
	    	isset($this->data['admin_mob_sanbox_chip_pin_user'])?$this->data['admin_mob_sanbox_chip_pin_user']:'' );
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_mob_sanbox_chip_pin_pass",
	    	isset($this->data['admin_mob_sanbox_chip_pin_pass'])?$this->data['admin_mob_sanbox_chip_pin_pass']:'' );



	    	// Mobile Live 	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_mob_live_chip_pin_user",
	    	isset($this->data['admin_mob_live_chip_pin_user'])?$this->data['admin_mob_live_chip_pin_user']:'' );
	    	
	    	Yii::app()->functions->updateOptionAdmin("admin_mob_live_chip_pin_pass",
	    	isset($this->data['admin_mob_live_chip_pin_pass'])?$this->data['admin_mob_live_chip_pin_pass']:'');


	    	Yii::app()->functions->updateOptionAdmin("admin_mob_live_chip_pin_user_id",$admin_id,
	    	isset($this->data['admin_mob_live_chip_pin_user_id'])?$this->data['admin_mob_live_chip_pin_user_id']:'' );

	    	Yii::app()->functions->updateOptionAdmin("admin_mob_live_chip_pin_password",$admin_id,
	    	isset($this->data['admin_mob_live_chip_pin_password'])?$this->data['admin_mob_live_chip_pin_password']:'' );

	    	Yii::app()->functions->updateOptionAdmin("admin_mob_live_chip_pin_client_id",$admin_id,
	    	isset($this->data['admin_mob_live_chip_pin_client_id'])?$this->data['admin_mob_live_chip_pin_client_id']:'' );	 
	    	...
	    	Yii::app()->functions->updateOptionAdmin("admin_chip_pin_fee",
	    	isset($this->data['admin_chip_pin_fee'])?$this->data['admin_chip_pin_fee']:'');
	    	
	    	Yii::app()->functions->updateOptionAdmin("adm_chip_pin_mobile_enabled",
	    	isset($this->data['adm_chip_pin_mobile_enabled'])?$this->data['adm_chip_pin_mobile_enabled']:'');
	    	
	    	Yii::app()->functions->updateOptionAdmin("adm_chip_pin_mobile_mode",
	    	isset($this->data['adm_chip_pin_mobile_mode'])?$this->data['adm_chip_pin_mobile_mode']:'');
	    	
	    	Yii::app()->functions->updateOptionAdmin("adm_chip_pin_mobile_clientid",
	    	isset($this->data['adm_chip_pin_mobile_clientid'])?$this->data['adm_chip_pin_mobile_clientid']:'');
	    ...
	    public function updateReview()
	    {
	    	...
	    	if ( $this->data['web_session_id']!=session_id()){	 
	    		$this->msg=Yii::t("default","Sorry! you cannot post directly to this action");
	    		return ;
	    	}	    		    	
	    ...
	    {	    		  
	    	...
	    	if ( $res=$DbExt->rst($stmt)){
	    		foreach ($res as $val) {	    			    			
	    			...
	    			if($val['client_name']=='')
	    			{
	    					$client_query = "SELECT `client_name` FROM `mt_guest_details` WHERE `order_id` = ".$val['order_id'];
	    					if($client_name = $DbExt->rst($client_query))
	    					{
	    						$val['client_name'] = isset($client_name[0]['client_name'])?$client_name[0]['client_name']:'';
	    					}
	    			}
	    			...
	    			$feed_data['aaData'][]=array(
	    			  ...
	    			  prettyFormat($val['bill_total'],$merchant_id),
	    			  ucwords($val['status']),
	    			  $date,
	    			  $action
	    		    );
	    ...
	    {	    	
    	    ...
    	    if ( Yii::app()->functions->isMerchantCommission($merchant_id)){    	    	
    	    	if ( FunctionsK::validateChangeOrder($this->data['order_id'])){
    	    		$this->msg=t("Sorry! you cannot change the order status of this order it has reference already on the withdrawals that you made");
    	    		return;
    	    	}    	    
    	    }	        	    
	    			...
	    			if (is_numeric($can_edit) && !empty($can_edit)){
		    			...
		    			if (is_array($date_interval) && count($date_interval)>=1){		    				
		    				if ( $date_interval['days']>$can_edit){
		    					$this->msg=t("Sorry! you cannot change the order status anymore. Order is lock by the website admin");
	    ...
	    {	    		    
	    	...
	    	if ( $res=$DbExt->rst($stmt)){
	    		...
	    		foreach ($res as $val) {	    			
	    			...
	    			if($val['client_name']=='')
	    			{
	    					$client_query = "SELECT `client_name`,client_contact_number FROM `mt_guest_details` WHERE `order_id` = ".$val['order_id'];
	    					if($client_name = $DbExt->rst($client_query))
	    					{
	    						$val['client_name'] = isset($client_name[0]['client_name'])?$client_name[0]['client_name']:'';
	    						$val['contact_phone'] = isset($client_name[0]['client_contact_number'])?$client_name[0]['client_contact_number']:'';
	    					}
	    			}
		...
		{		    
		    ...
		    if ( !Yii::app()->functions->validateUsername($this->data['username']) ){
			    ...
			    } else $this->msg=Yii::t("default","Sorry! we cannot add your information. Please try again later");
		    } else $this->msg=Yii::t("default","Sorry! your username is alread been taken.");
		}
		...
		{		   
		   ...
		   if (isset($this->data['activation_code']) && isset($this->data['token'])){
		      ...
		      } else $this->msg=Yii::t("default","Sorry! we cannot find your information.");
		...
		{ 
			...
			if (isset($this->data['payment_opt'])){
				...
				elseif ($this->data['payment_opt']=="cpy"){  //citypay
		...
		$city_pay_details = Yii::app()->functions->getCityPayConnectionAdmin();
		 
		$city_pay_details_mode = $city_pay_details['mode'];
		 
		if((($city_pay_details[$city_pay_details_mode]['user'])&&($city_pay_details[$city_pay_details_mode]['psw']))!='')
        ...
        define('MERCHANT_ID', $city_pay_details[$city_pay_details_mode]['user']); //31116985       wild card id 
        define('LICENCE_KEY', $city_pay_details[$city_pay_details_mode]['psw']);	// SMR4S4UNEX94HMQ1	wildcard license key 
        ...
        $request['amount'] = $package_price * 100 ;
        ...
        if (isset($this->data['renew'])){
						$request['config']['redirect_success'] ="http://".$_SERVER['HTTP_HOST'].Yii::app()->request->baseUrl."/store/merchantsignup/Do/step3a/internal-token/$token/renew/1/package_id/".$this->data['package_id'];
					} else {
				       $request['config']['redirect_success']="http://".$_SERVER['HTTP_HOST'].Yii::app()->request->baseUrl."/store/merchantsignup/Do/step3a/internal-token/$token";
					}
...
if (!empty($httpsResponse))
{
	...
	//  $this->redirect($url);	
 	  ...
 	  exit;
 	 $this->code=1;
			  	  	  $this->msg=Yii::t("default","Please wait while we redirect you to Citypay.");
		...
		public function add_bg_image()
		{	                                
		    $Validator=new Validator;
			$req=array(
			  'bgimage'=>Yii::t("default","Image is required"),                            
                            'img_alt'=>Yii::t("default","Image alt is required"),                             
			);		
			$Validator->required($req,$this->data);
			if ($Validator->validate()){
				$params=array(				  
				  'img_alt'=>$this->data['img_alt'],
				  'src'=>$this->data['bgimage'],
                                  'status'=>$this->data['img_status'],  
				);				
                                
				
			   if (empty($this->data['id'])){	
			    	if ( $this->insertData("{{bground_img}}",$params)){
			    		    $this->details=Yii::app()->db->getLastInsertID();
				    		$this->code=1;
				    		$this->msg=Yii::t("default","Successful");				    		
				    	}
				  ...
				  'src'=>$this->data['bgimage'],
						...
						$res = $this->updateData('{{bground_img}}',$params,'id',$this->data['id']);  
                                
						if ($res){
							$this->code=1;
			                $this->msg=Yii::t("default",'Background Image updated.');  
					} else $this->msg=Yii::t("default","ERROR: cannot update");
			    }	
			} else $this->msg=$Validator->getErrorAsHTML();		
		}
		
		public function add_external_json()
		{	                                
		    $Validator=new Validator;
			$req=array(
			  'merchant_list'=>Yii::t("default","Merchant is required"),                            
              'website_address'=>Yii::t("default","Website Address alt is required"),                             
			);		
			$Validator->required($req,$this->data);
			if ($Validator->validate()){
				$params=array(				  
				  'merchant_list'=>json_encode($this->data['merchant_list']),
				  'websiteaddress'=>$this->data['website_address'],
                  'status'=>$this->data['status'],  
				);				
                                
				
			   if (empty($this->data['id'])){	
			    	if ( $this->insertData("{{external_json}}",$params)){
			    		    $this->details=Yii::app()->db->getLastInsertID();
				    		$this->code=1;
				    		$this->msg=Yii::t("default","Successful");				    		
				    	}
                  ...
                  'status'=>$this->data['status'],  
						...
						$res = $this->updateData('{{external_json}}',$params,'id',$this->data['id']);  
		...
		public function add_merchant_deals()
		{	                                
		    $Validator=new Validator;
			$req=array(
			  'merchant_list'=>Yii::t("default","Merchant is required"),                                         
			...
			if ($Validator->validate()){
				$params=array(				  
                  ...
                  'status'=>$this->data['status'],  
						...
						$res = $this->updateData('{{admin_enable_deals}}',$params,'id',$this->data['id']);  
		...
		public function merchant_add_deals()
		{	     
			...
			$req=array(
			  'deal_title'=>Yii::t("default","Deal Title is required"),  
			  'deal_description'=>Yii::t("default","Deal Desc is required"),  
			  'from_date'=>Yii::t("default","From Date is required"),  
			  'to_date'=>Yii::t("default","To Date is required"),                                         
			);		
			$Validator->required($req,$this->data);
			if ($Validator->validate()){
				...
				if($this->data['deals_type']==0||$this->data['deals_type']==1)
				{
					// echo $this->data['bogo_multi_size']; 
					$bogo_multi_size = explode("___",$this->data['bogo_multi_size']);
					
					$all_items = $this->data['item'];

					$common_array = array();
					

					foreach ($all_items as $individual_items) 
					{
						// print_r($all_items);
						foreach($bogo_multi_size as $individual_values)
						{
							/* print_r($bogo_multi_size);
							exit; */
							$individual_values = json_decode($individual_values,true);
							if(in_array($individual_values['item_id'],$all_items))
							{
								 
								/* echo $individual_values['item_id'] ."\n\n";
								print_r($common_array);
								echo "\n\n";  */

								if(!in_array($individual_values['item_id'],$common_array))
								{
									array_push($common_array,$individual_values['item_id']);
									/*print_r($individual_values);
									exit; */
							$items_array[$individual_values['item_id']] = array('size'=>$individual_values['size'],'size_id'=>$individual_values['size_id']);	
								}
							}
				...
				$params=array(				  
				  'title'=>$this->data['deal_title'],				  
                  'description'=>$this->data['deal_description'],  
                  'deal_type'=>isset($this->data['deals_type'])?$this->data['deals_type']:'',
                  'item_list'=>isset($this->data['item'])?json_encode($this->data['item']):'',
                  'item_sizes'=>$items_array,
                  'spend_for'=>isset($this->data['spend_amount'])?$this->data['spend_amount']:'',
                  'discount'=>isset($this->data['discount_percent'])?$this->data['discount_percent']:'',
                  ...
                  'status'=>$this->data['status']                
				   ...
				   if (empty($this->data['id'])){	
			    	if ( $this->insertData("{{merchant_deals}}",$params)){
			    		    $this->details=Yii::app()->db->getLastInsertID();
				    		$this->code=1;
				    		$this->msg=Yii::t("default","Successful");				    		
				    	}
					...
					if (isset($this->data['id']))
					{
						...
						if ($this->data['deals_type'] == 0 || $this->data['deals_type'] == 1)
						{
							...
							{ 
								...
								{ 
									...
									if (in_array($individual_values['item_id'], $all_items))
									{
										...
										{
											array_push($common_array, $individual_values['item_id']);
											$items_array[$individual_values['item_id']] = array(
												'size' => $individual_values['size'],
												'size_id' => $individual_values['size_id']
											);
										}										
									}
								}
							...
							$stmt = " SELECT `item_sizes` FROM `mt_merchant_deals` WHERE `id` =  " . $this->data['id'];
							$result = $this->rst($stmt);
							if(isset($result[0]['item_sizes']))
							{
								$result = json_decode($result[0]['item_sizes'],true);
							}							 
							foreach($result as $key => $value)
							{ 
								$existing_values_key = $key;
								if (in_array($existing_values_key, $all_items))
								{
									if (!isset($items_array[$existing_values_key]))
									{
										$items_array[$existing_values_key] = array(
											'size' => $result[$existing_values_key]['size'],
											'size_id' => $result[$existing_values_key]['size_id']
										);
									}
								}
							}	
                  ...
                  'status'=>$this->data['status']                
						...
						$res = $this->updateData('{{merchant_deals}}',$params,'id',$this->data['id']);  
                                
						if ($res){
							$this->code=1;
			                $this->msg=Yii::t("default",'Deals updated.');  
					} else $this->msg=Yii::t("default","ERROR: cannot update");
			    }
			}
 			 else $this->msg=$Validator->getErrorAsHTML();		
		...
		{	                                
			...
			$req=array(
			  'bgimage'=>Yii::t("default","Image is required"),                            
              'category_type'=>Yii::t("default","Image Category is required"),
              'tooltip_text'=>Yii::t("default","Tooltip text is required"),                             
			);		
			$Validator->required($req,$this->data);
			if ($Validator->validate()){
				$params=array(				  
				  'category_type'=>$this->data['category_type'],
				  'img_url'=>$this->data['bgimage'],
                  'status'=>$this->data['img_status'],  
                  'tooltip_text'=>$this->data['tooltip_text'],  
				);				
                                
				
			   if (empty($this->data['id'])){	
			    	if ( $this->insertData("{{item_category}}",$params)){
			    		    $this->details=Yii::app()->db->getLastInsertID();
				    		$this->code=1;
				    		$this->msg=Yii::t("default","Successful");				    		
				    	}
                  ...
                  'tooltip_text'=>$this->data['tooltip_text'],    
						...
						$res = $this->updateData('{{item_category}}',$params,'id',$this->data['id']);  
                                
						if ($res){
							$this->code=1;
			                $this->msg=Yii::t("default",'Image Category updated.');  
					} else $this->msg=Yii::t("default","ERROR: cannot update");
			    }	
			} else $this->msg=$Validator->getErrorAsHTML();		
		}
			...
			if (isset($this->data['status'])){
				...
				if ( $_SESSION['kr_merchant_user_type']=="merchant_user"){
					...
					if (!$data=Yii::app()->functions->getMerchantUserInfo($user_id)){
						$this->msg=t("Sorry! you dont have permission");
						return ;
	    ...
	    {	    	
	    	...
	    	if ($validator->validate()){    		
	    		...
	    		foreach ($this->data as $key=>$val) {    			    			
	    			if($key=="phone") { $key = "Order Reference Number ";   } 
	        ...
	        if (isset($this->data['email'])){
	    		...
	    		if ($social_info=yii::app()->functions->accountExistSocial($this->data['email'])){      		    
		            ...
		            $this->data['password_md5']=addslashes($social_info[0]['password']);
		              ...
		              if(isset($_SESSION['FACEBOOK_EMAIL']))
	    ...
	    public function forgotPassword()
	    {
			...
			if ($Validator->validate()){
				...
				} else $this->msg=Yii::t("default","Sorry! your Email address does not exist in our records.");
			} else $this->msg=$Validator->getErrorAsHTML();
	    }
	    ...
	    {	    	
	    	if ( isset($this->data['email_address'])){
	    		...
	    		} else $this->msg=Yii::t("default","Sorry! we cannot find your email address.");
	    ...
	    {	    	
	    	if (isset($this->data['lost_password_code'])){
	    		...
	    		} else $this->msg=Yii::t("default","Sorry! your verification code is incorrect");
	    	} else $this->msg=Yii::t("default","ERROR: missing parameters");    
	    }
...
$country=array_key_exists($val['country_code'],(array)$country_list)?$country_list[$val['country_code']]:$val['country_code'];
	    ...
	    public function SMSAlertSettings()
	    {
			...
			Yii::app()->functions->updateOption("sms_alert_tbl_rej",
	    	isset($this->data['sms_alert_tbl_rej'])?$this->data['sms_alert_tbl_rej']:''
	    	,$merchant_id);
	    	
	    	Yii::app()->functions->updateOption("sms_alert_tbl_accept",
	    	isset($this->data['sms_alert_tbl_accept'])?$this->data['sms_alert_tbl_accept']:''
	    	,$merchant_id);
	    ...
	    {	    	
	    	...
	    	if ( $this->data['send_to']==1){	    		
	    		if ( $available_credit<$this->data['total_customer']){
	    			$this->msg=Yii::t("default","Sorry! your SMS Credits is low. Please purchase SMS credit to continue send SMS. Thank you");
	    			return ;
	    		}
...
$last_login=$val['last_login']=="0000-00-00 00:00:00"?"":date('M d,Y G:i:s',strtotime($val['last_login']));  */
		...
		{			
			...
			if (FunctionsV3::hasModuleAddon("pointsprogram")){
				if (isset($_SESSION['pts_redeem_amt']) && $_SESSION['pts_redeem_amt']>0.01){
					$this->msg=t("Sorry! you cannot apply voucher when you have already redeem a points");
					return ;
				}
			}
			...
			if (isset($this->data['voucher_code'])){
				if ( $res=Yii::app()->functions->getVoucherCodeNew($this->data['voucher_code'],$this->data['merchant_id']) ){
					...
					// } else $this->msg=Yii::t("default","Sorry! you have already use this voucher code");
		...
		{	
            ...
            if(isset($this->data['table_booking_time'])&&!empty($this->data['table_booking_time']))
            {
            	$booking_time = explode("-",$this->data['table_booking_time']);
            	if(isset($booking_time[0]))
            	{
            		$this->data['booking_time'] = $booking_time[0];
            	}
            }             
			
			// $this->data['booking_time'] = "09:01 AM";	
			...
			if ( !Yii::app()->functions->isMerchantOpenTimes($merchant_id,$full_booking_day,$booking_time))
			{
				$this->msg=t("Sorry! we are closed on"." ".date("F,d Y h:ia",strtotime($full_booking_time))).
		    ...
		    if ( $is_holiday==true){
		    	$merchant_close_msg_holiday=!empty($merchant_close_msg_holiday)?$merchant_close_msg_holiday:t("Sorry! we are on holiday on")." ".date("F d Y",strtotime($now));
		    	$this->msg=$merchant_close_msg_holiday;
		    	return ;
		    }
                   
                    $number_of_guest = $this->data['number_guest'];
                    if($number_of_guest<1)
                        {
                            $this->msg=t("Please enter a valid Number Of Guests");
                            return ;
                        }
                    
                    $guest_details = Yii::app()->functions->merchant_remaining_table($number_of_guest,$merchant_id,$this->data['table_booking_time']);
                    // print_r($guest_details);  $this->data['table_booking_time']
                    
                    if($guest_details['result']=='false')
                    {
                        if($guest_details['remaining_seats']==0)
                        {
                        	if($fully_booked_msg=Yii::app()->functions->getOption("fully_booked_msg",$merchant_id))
                        	{
                        		$this->msg=t($fully_booked_msg);
                            	return ;
                        	}
                            $this->msg=t("Sorry we are fully booked for the Slot , Please try the Next Slot ");
                            return ;
                        }
                        else
                        {
                            $this->msg=t("You can able to book upto ".$guest_details['remaining_seats']." seats only");
                            return ;                            
                        }                        
                    }                   
		     
                        
		   /* if (!Yii::app()->functions->bookedAvailable($merchant_id)) 11-09-2017
		    {
			...
			//  'booking_time'=>isset($this->data['booking_time'])?$this->data['booking_time']:'',
			    'booking_time'=>isset($this->data['table_booking_time'])?$this->data['table_booking_time']:'',
		...
		public function bookATableNewconcept()
		{	
			...
			$full_booking_time=$this->data['selected_date'];
			$user_selected_time = $this->data['user_selected_time'];
			$no_of_guests       = $this->data['no_of_guests'];
            ...
            $booking_query = "SELECT SUM(`number_guest`) as total_count FROM `mt_bookingtable` WHERE decliened = 1 AND `merchant_id` = $merchant_id AND `booking_time` LIKE '%".$user_selected_time."%' AND `date_booking` = '".$db_date."'";
			...
			//	echo $no_of_guests." ".$seats_available;
            ...
            {                       
				...
				$params=array(
				  'merchant_id'=>isset($this->data['merchant-id'])?$this->data['merchant-id']:'',
				   'client_id'=>$client_id,
				  'number_guest'=>isset($this->data['no_of_guests'])?$this->data['no_of_guests']:'',
		...
		/*	$merchant_sms_booking_receiver = Yii::app()->functions->getOption("merchant_sms_booking_receiver",$merchant_id);
			if(!empty($merchant_sms_booking_receiver))
			{ */
				//$sms_msg = $this->data['message'];

				if ($this->data['status']=="approved")
					{
						 $sms_msg = "Hi ".$params['booking_name'].", Your Booking for ".$params['number_guest']." on ".$params['booking_time']." has been Approved ";						 
					} elseif ($this->data['status']=="denied")
					{
						 $sms_msg = "Hi ".$params['booking_name'].", Sorry Your Booking for ".$params['number_guest']." on ".$params['booking_time']." has been Declined \n Sorry for the inconvenience Caused ";
					}  
					...
					if ($resp= Yii::app()->functions->sendSMS($to,$sms_msg)){							
						if ( $resp['msg']=='process'){
						    ...
						    $params=array(
						      ...
						      'client_id'=>0,
						      'client_name'=>$params['booking_name'],
						      'contact_phone'=>$to,
						      'sms_message'=>$sms_msg,
						      'status'=>'process',
						...
						} else $this->msg=Yii::t("default",$resp['msg']);
					} else $this->msg=Yii::t("default",$resp['msg']);
				} else $this->msg=Yii::t("default",'No SMS Credit');
		...
		{		
			...
			$data['mercahnt_id']			 = $merchant_id;

			// $data['alloted_date'] 			 = date('Y-m-d',strtotime($this->data['merchant_picked_date']));

			if(isset($this->data['enable_slot']) && sizeof($this->data['enable_slot'])>0 )
			{
				 $data['timings'] = json_encode($this->data['enable_slot']);
			}

			if(isset($this->data['seating_capacity']) && sizeof($this->data['seating_capacity'])>0 )
			{
				 $data['seat_capacity'] = json_encode($this->data['seating_capacity']);
			}

			$DbExt=new DbExt;
			// $stmt = " SELECT * FROM `mt_table_booking` WHERE alloted_date = '".$data['alloted_date']."' AND `mercahnt_id` =  ".$merchant_id;
			$stmt = " SELECT * FROM `mt_table_booking` WHERE   `mercahnt_id` =  ".$merchant_id;
			if($res=$DbExt->rst($stmt))
			{
				//	$DbExt->updateData("{{table_booking}}",$data,'mercahnt_id',$merchant_id);

				$command = Yii::app()->db->createCommand();

				$res = $command->update('{{table_booking}}' , $data , 
				                     'mercahnt_id=:mercahnt_id',
				                     array(				                      
				...
				//	$res = $command->update('mt_table_booking' , $data , array('mercahnt_id'=> $merchant_id ));
			}
			else
			{
				$DbExt->insertData("{{table_booking}}",$data);
			}			 

			Yii::app()->functions->updateOption("merchant_table_menu",
	    	isset($this->data['photo'])?json_encode($this->data['photo']):'',$merchant_id);	 

			Yii::app()->functions->updateOption("merchant_spl_table_menu",
	    	isset($this->data['spl_photo'])?json_encode($this->data['spl_photo']):'',$merchant_id);	 
	    	...
	    	Yii::app()->functions->updateOption("merchant_sms_booking_receiver",
	    	isset($this->data['merchant_sms_booking_receiver'])?$this->data['merchant_sms_booking_receiver']:''
	    	,$merchant_id);
