		...
		return array(  
            ...
            /*      array('visible'=>$this->AA('sms'),
                 'tag'=>'sms','label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default",'SMS'),
                   'itemOptions'=>array('class' => 'has_sub'), 'items'=>array(
    		...
    		if (is_array($cart_item) && count($cart_item)>=1){
			          	 ...
			          	 foreach ($val['sub_item'] as $cat_id=>$val_sub) {				          	 	
			          	 	...
			          	 	foreach ($val_sub as $addon_row=> $val_subs) {
						        ...
						        $htm.='<div class="clear"></div>';*/				          	 	
			          	 		...
			          	 		$htm .='<td><span class="addon_item">'.ucwords(qTranslate($subcat_list[$cat_id],'subcategory_name',
						        $subcategory_trans)).'</span>';
										//<span>'.$addon_qty.'x</span>';			          	 					          	 		
			          	 		//$htm.='<div class="a">'.$addon_qty.'x</div>';
								/*$htm.='<div class="b uk-text-muted">
											'."$addon_raw_price ".ucwords(qTranslate($val_subs[2], 'sub_item_name',$addon_name_trans))
								      .'</div>';*/
								$htm .='<span>'.ucwords(qTranslate($val_subs[2], "sub_item_name",$addon_name_trans)).'</span>';	 
								$htm .='</td>';	  
								$htm .='<td> <span>'.displayPrice(baseCurrency(),prettyFormat($addon_raw_price)).'<span></td>';		  
						          /*$htm.='<div class="manage">';							             
	    	   ...
	    	   $htm.="</div>"; */
 			   $htm.= '<tr>
							<td colspan="4">
								'.t("Total").'
								<span class="pull-right">'.displayPrice(baseCurrency(),prettyFormat($total,$mid)).'</span>
							</td>
						</tr>'; 
 			   $htm .= ' </tbody>
					 </table>';	
...
$menu_html.="</li>";
   ...
   /* public function getMerchantListOfPaymentGateway()
	...
	{
		  $parish_array = array('0'=>$first_str,'1'=>'Saint Helier','2'=>'Grouville','3'=>'Saint Brélade','4'=>'Saint Clement',
							  '5'=>'Saint John','6'=>'Saint Lawrence','7'=>'Saint Martin','8'=>'Saint Mary',
							  '9'=>'Saint Ouen','10'=>'Saint Peter','11'=>'Saint Saviour','12'=>'Trinity',);
		  return $parish_array;
	} 
