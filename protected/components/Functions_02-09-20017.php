...
{	
	...
	{						
	 ...
	 public function merchant_remaining_table($number_of_guest,$merchant_id='')
        {
 ...
 public function Get_category_img($id='')
	...
	public function merchantMenu()
	{
		...
		return array(  
		    ...
		    'items'=>array(
                ...
                array('visible'=>$this->hasMerchantAccess("deliverableparish"),'tag'=>"deliverableparish",'label'=>'<i class="fa fa-list-alt"></i>'.Yii::t("default","Deliverable Parish"),
	...
	public function hasMerchantAccess($tag='')
	{
		...
		$tag_paymentgateway=array(
		  ...
		  'atz','obd','pyp','stp','mcd','ocr','btr','cpy'
		);				
	 ...
	 public function getmerchant_deal($id='')
    {
    	...
    	$DbExt=new DbExt;
	    $stmt="
	    	SELECT * FROM `mt_merchant_deals` WHERE `id` = ".$id." AND `status` = 0 
			AND
			merchant_id ='$merchant_id'
			LIMIT 0,1
		";		
...
public function displayOrderHTML($data = '', $cart_item = '', $receipt = false, $new_order_id = '')
{
	...
	if (isset($cart_item))
	{
		if (is_array($cart_item) && count($cart_item) >= 1)
		{
			...
			foreach($cart_item as $key => $val)
			{
				...
				if ($receipt == false):
					// 	$data_row += 1;
					$htm.= '<span>
						 		 <a href="javascript:;" class="edit_item" title="edit" data-row="' . $data_row . '" rel="' . $val['item_id'] . '" >
				...
				if (!empty($buy_one_get_one_list))
				{
					...
					{						
						...
						{
							$explode_size = explode("|", $deal_items_with_size[$val['item_id']]->size[0]);
							...
							if (strtolower($size_words) == strtolower($explode_size))
							{
								...
								if ($receipt == false):
									$htm.= '<span style="display:none">
										 		 <a href="javascript:;" class="edit_item" data-row="' . $data_row . '" rel="' . $val['item_id'] . '" >
							                        <i class="ion-compose"></i>
							                     </a>
											   </span> ';
									$htm.= '<span style="display:none">
						...
						else
						{
							...
							if ($receipt == false):
								$htm.= '<span style="display:none">
										 		 <a href="javascript:;" class="edit_item" data-row="' . $key . '" rel="' . $val['item_id'] . '" >
							                        <i class="ion-compose"></i>
							                     </a>
											   </span> ';
								$htm.= '<span style="display:none">
			...
			if (isset($deals_spend_for_get_prd) && isset($deals_buy_over_get_prd))
			{
				if ($deals_spend_for_get_prd != 0)
				{
					if ($total > $deals_spend_for_get_prd)
					{
						...
						{
							foreach($deals_buy_over_get_prd as $free_item_list)
							{
								$get_free_prds_query = "SELECT `item_name` FROM `mt_item` WHERE `item_id` =  " . $free_item_list;
								$item_name = $DbExt->rst($get_free_prds_query);
								if (isset($item_name[0]['item_name']))
								{
			...
			if ($has_voucher == TRUE)
			{
				...
				$htm.= '<tr>
								<td colspan="4">
									Sub Total <span class="pull-right cart_subtotal">' . displayPrice(baseCurrency() , prettyFormat($subtotal + $less_voucher, $mid)) . '</span>
								</td>
							</tr>';
				if ($receipt == TRUE)
				{
				...
				$htm.= '<tr>
								<td colspan="4">
									Sub Total <span class="pull-right">(' . displayPrice(baseCurrency() , prettyFormat($subtotal, $mid)) . ')</span>
								</td>
							</tr>';
			}
			else
			{
				...
				$htm.= '<tr>
								<td colspan="4">
									Sub Total
									<span class="pull-right cart_subtotal">' . displayPrice(baseCurrency() , prettyFormat($subtotal, $mid)) . '</span>
								</td>
							</tr>';
			}
			if (!empty($delivery_charges))
			...
			<input type="hidden" value="' . displayPrice(baseCurrency() , prettyFormat($discount_price, $mid)) . '" id="hidden_deals_discount" name="hidden_deals_discount" />
								</td>
								</tr>';
			...
			if (sizeof($deals_discount_price) > 1)
			{
				...
				foreach($discount_rates as $spend_for => $discount_rate)
				{
					...
					if ($total >= $spend_for)
					{
						$discount_price = $total * ($discount_rate / 100);
		...
		<span class="pull-right">' . displayPrice(baseCurrency() , prettyFormat($discount_price, $mid)) . '</span>
						...
						$final_total = $total - $discount_price;
					}
				}
...
function displayOrderHTMLOriginal($data = '', $cart_item = '', $receipt = false, $new_order_id = '')
{
     ...
     public function clientTblBookingOrderDetails($client_id='')
    {
        ...
        SELECT bt.*,mt.restaurant_name FROM {{bookingtable}}
    	 as bt
        INNER JOIN 
        {{merchant}} as mt
