...
class FunctionsV3
{
	...
	public static function getPerPage()
	{
	 ...
	 public static function getPerPage_table()
	{
		return 12;		
	}
	...
	public static function getDesktopLogo()
	{
		...
		return assetsURL()."/images/logo.png";
	}
    ...
    {    
    	...
    	switch ($service) {
    		case 1:
    			$htm.='<li>'.t("Delivery").' <i class="fa fa-check ok"></i></li>';
    			$htm.='<li>'.t("Pickup").' <i class="fa fa-check ok"></i></li>';
    			break;    		 
    		case 2:
    			$htm.='<li>'.t("Delivery").' <i class="fa fa-check ok"></i></li>';    				
    			break; 
    ...
    public static function  searchGetFilter($getdata='',$rest_id = '')
    {
    	$and='';
    	//dump($getdata);
		...
		if (isset($getdata['filter_special_offer'])){
			if (!empty($getdata['filter_special_offer'])){
 				
			    $and.=" AND (SELECT count(offr.offers_id) FROM
					 {{offers}} offr
					 WHERE
					 offr.status in ('publish','published')
					 AND
					 now() >= offr.valid_from and now() <= offr.valid_to
					 AND offr.merchant_id = a.merchant_id
					 LIMIT 0,1) > 0 "; 
 			}
		} 
		if (isset($getdata['filter_delivery_free_chkbx'])){
			if (!empty($getdata['filter_delivery_free_chkbx'])){
			    $and .=" AND (delivery_charges = '' OR delivery_charges IS NULL) ";
			}
		}   
    ...
    {        
		...
		switch ($stype) {
			...
			case "kr_postcode":   
			   ...
			   if($post_code == '')$query=" post_code != '' ";
			   
 			   $parish=isset($getdata['parish'])?$getdata['parish']:'';
			   if(!empty($parish))
			   {
			   	  $query .= " AND parish = ".$parish.""; 
			   }  

			   break;
			 
			  case "kr_search_book_a_table":                           
		 	  $admin_permission = self::chk_admin_tbl_sts(); 			  
			   if($admin_permission[0]['option_value']!=2) {  			  
                               $merchant_id    = '';
                               $DbExt = new DbExt(); 
                               $check_query = "SELECT merchant_id FROM `mt_option` WHERE  `option_name`='merchant_table_booking' AND option_value = 'yes' ";
							   $query =" merchant_id NOT IN (0) ";
                               if($res=$DbExt->rst($check_query))
							   { 
								   if(!empty($res)) 
										{                                 
									...
									$merchant_id = rtrim($merchant_id,",");
									$query=" merchant_id NOT IN ($merchant_id) ";
                                   		 }    
								}
			  }
			   break;  
 		...
 		{ 			 
	    	...
	    	{	    		 
	    		$get_all_deliverable_merchant = " SELECT `merchant_id`,`services`,`deliver_to_all_parish` FROM `mt_parish_deliver_settings`
				WHERE merchant_id IN (SELECT merchant_id FROM  `mt_merchant` WHERE  `status` =  'active'AND  `is_ready` =2	)  ";
	    		$DbExt = new DbExt(); 
	    		...
	    		{
	    			$append_sql =	'WHERE a.merchant_id IN ('.$merchant_list_array.') ';
	    		}

				if(!empty($merchant_list_array)&&($parish==0))
	    		{
	    			$append_sql =	'';
	    		}	    		
    			...
    			if(!empty($merchant_list_array)&&($parish!=0))
    			{
					$append_sql  .= 'OR parish = '.$parish;	    				
    			}	

    			if(empty($merchant_list_array)&&($parish!=0))
    			{
					$append_sql  .= 'WHERE parish = '.$parish;	    				
    			}	
				...
				if (isset($_GET['filter_cuisine']))
				{
					...
					if (is_array($filter_cuisines) && count($filter_cuisines)>=1){
						...
						if (!empty($filter_cuisine)){
							if($append_sql!='')
							{
								$append_sql.=" AND (cuisine $filter_cuisine) ";	
							}
							else
							{
								$append_sql.=" WHERE (cuisine $filter_cuisine) ";	
							}						   
						}
					}
				}	 
			   	...
			   	$merchant_name= isset($getdata['restaurant_name'])?$getdata['restaurant_name']:'';				
			   	if($merchant_name!='')
			   	{
			   			if (preg_match("/'/i", $merchant_name )) 
						{
							$merchant_name=substr($merchant_name,0, strpos($merchant_name,"'"));
							$merchant_name_sql .=" restaurant_name LIKE '%$merchant_name%' ";
						} 
						else 
						{
							$merchant_name_sql .=" restaurant_name LIKE '%$merchant_name%' ";
						...
						if($append_sql!='')
						{
							$append_sql.=" AND ".$merchant_name_sql;	
						}
						else
						{
							$append_sql.=" WHERE ".$merchant_name_sql;	
						}	
				...
				if (isset($_GET['filter_minimum']))
				{
					if (is_numeric($_GET['filter_minimum']))
					{
						$filter_minimum_sql.=" CAST(minimum_order as SIGNED) <='".$_GET['filter_minimum']."' ";
					}		
				}	

				if($append_sql!='')
				{
					if($filter_minimum_sql!='')
					{
						$append_sql.=" AND ".$filter_minimum_sql;	
					}
				}
				else
				{
					if($filter_minimum_sql!='')
					{
						$append_sql.=" WHERE ".$filter_minimum_sql;	
					}					
				}	

				if($append_sql=='')
				{
					$append_sql.=" WHERE a.`status`='active' ";	
				}
				else
				{
					$append_sql.=" AND a.`status`='active' ";		
				}
				...
				{			
					...
					{
						case 1:
							$append_sql.= "AND ( service='1' OR service ='2' OR service='3')";
							break;			
						case 2:
							$append_sql.= "AND ( service='1' OR service ='2')";
							break;
						case 3:
							$append_sql.= "AND ( service='1' OR service ='3')";
							break;		
						case 4:
							$append_sql.= "AND service='4' ";
							break;		
						default:
							break;
					}		
	    ...
	    // echo $stmt;   
		/* echo " append_sql  : " .$append_sql ." <br /> ";
		echo " sort_combine : ".$sort_combine." <br />"; */
		...
		/* echo $stmt;   
		 ...
		 /* 	$append_sql = '';
	    		...
	    		if(!empty($merchant_list_array))
	    		{
	    			$append_sql =	'a.merchant_id IN ('.$merchant_list_array.') ';
	    		}
	    		...
	    		if($parish!=0)
	    		{
	    			...
	    			$append_sql .= $condition.' parish = '.$parish;
	    		}
	    		else
	    		{
...
*/ 
	...
	public static function chk_admin_tbl_sts()
    {
        $query = "SELECT option_value FROM  `mt_option` WHERE  `merchant_id` = 0 AND  `option_name` = 'merchant_tbl_book_disabled'";
        $DbExt = new DbExt();  		
        if ($res=$DbExt->rst($query))
        { 
         return $res;   
        }
    }
    ...
    public static function merchantOpenTag($merchant_id='')
    {
	    ...
	    $merchant_preorder= Yii::app()->functions->getOption("merchant_preorder",$merchant_id);
	     ...
	     $now = new DateTime();
	     $now->setTimezone(new DateTimeZone('Europe/Jersey'));

	     $current_date =$now->format('Y-m-d');
	     $current_time = $now->format('H:i');
	     $today = strtolower(date('l', strtotime($current_date)));
	     $opened_now = false ;
	     if($mechant_opening_time = Yii::app()->functions->get_merchant_splitup_time($merchant_id))
	     {
	     	$timings = $mechant_opening_time;	
	     	$merchant_opening_days = '';
	     	$merchant_morning_open = '';
	     	$merchant_morning_closed = '';
	     	$merchant_evening_open = '';	
	     	$merchant_evening_closed = '';
	     	//$opened_now = false ;
	     	foreach ($timings as $key => $value) 
	     	{	 
	     		if($value['option_name']=="stores_open_day")
	     		{
	     			$merchant_opening_days =  json_decode(stripslashes($value['option_value']),true);
	     		}
	     		if($value['option_name']=="stores_open_starts")
	     		{
	     			$merchant_morning_open = json_decode(stripslashes($value['option_value']),true);
	     		}
	     		if($value['option_name']=="stores_open_ends")
	     		{
	     			$merchant_morning_closed = json_decode(stripslashes($value['option_value']),true);	
	     		}
	     		if($value['option_name']=="stores_open_pm_start")
	     		{
	     			$merchant_evening_open = json_decode(stripslashes($value['option_value']),true);
	     		}
	     		if($value['option_name']=="stores_open_pm_ends")
	     		{
	     			$merchant_evening_closed = json_decode(stripslashes($value['option_value']),true);
	     		}	 
	     	}

	     	if(sizeof($merchant_opening_days)>0)
	     	{	 	     		 
	     		if(in_array($today,$merchant_opening_days))
	     		{	
	     			if(isset($merchant_morning_open[$today])&&isset($merchant_morning_closed[$today]))
	     			{	     				 
	     			 if($opened_now = Yii::app()->functions->checkBetweenTime($current_time,$merchant_morning_open[$today],$merchant_morning_closed[$today]))
	     			 {	     			 	
	     			 	
	     			 }
	     			 else
	     			 {	     			 	
	     			 	$opened_now = Yii::app()->functions->checkBetweenTime($current_time,$merchant_evening_open[$today],$merchant_evening_closed[$today]);	
	     			 }	     			  
	     			}		     			 
	     		}
     		}
	     	...
	     	if(!isset($opened_now)){ $opened_now = false;}
	     // print_r($timings);  option_name option_value  stores_open_day     stores_open_starts  stores_open_ends 
	     
			$is_holiday=false;
	        if ( $m_holiday=Yii::app()->functions->getMerchantHoliday($merchant_id))
	        {  
	      	   if (in_array($current_date,(array)$m_holiday))
	      	   {
        ...
        if ($is_merchant_open&&$opened_now){
        	$tag='<span class="label label-success">'.t("Open").'</span>';
        } else {
        	if ($merchant_preorder){        		
        		$tag='<span class="label label-warning">'.t("Pre-Order").'</span>';
        	} else {
        		$tag='<span class="label label-danger">'.t("Closed").'</span>';
        	}
        }      
    ...
    public static function checkMerchantstatus($merchant_id='',$now='')
    {
    	$is_merchant_open = Yii::app()->functions->isMerchantOpen($merchant_id); 
	    $merchant_preorder= Yii::app()->functions->getOption("merchant_preorder",$merchant_id);
    ...
    public static function getPickupEstimation($merchant_id='')
    {
    	$pickup_est=Yii::app()->functions->getOption("merchant_pickup_estimation",$merchant_id);
    	if (empty($pickup_est)){
    ...
    {
    	?>
			<h2 class="sub-title"><?php echo t($title)?></h2>   
    	<?php
    }
    ...
    public static function getAdminPaymentList()
    {
    	...
    	if (getOptionA('admin_enabled_citypay')==""){
    		$payment_available[]='cpy';
    	}   
    	if (getOptionA('admin_enabled_chip_pin')==""){
    		$payment_available[]='cpn';
    	}   
    ...
    /*    $is_commission=false;
		...
		{	 
			...
			if ( getOption($merchant_id,'merchant_disabled_citypay')==""){
				$payment_available[]='cpy';
			}  
			if ( getOption($merchant_id,'enabled_paypal')=="yes"){
				$payment_available[]='pyp';
			} 
			if ( getOption($merchant_id,'mt_chip_pin_mobile_enabled')=="yes"){
				$payment_available[]='cpn';
			} 
			
			/*echo $merchant_id;
			 print_r(getOption($merchant_id,'merchant_enabled_citypay'));
    ...
    public static function displayCashAvailable($merchant_id='',$echo=true)
    {
		...
		if (array_key_exists('cod',(array)$new_payment_list)){
			if ($echo){				
				echo "<p>".t("Cash on delivery available")."</p>";
			} else return true;
		}
		return false;
    }
    ...
    public static function getTopMenuActivated()
    {
		...
		if(empty($theme_top_menu)){
			$theme_top_menu=array(
			  'browse','resto_signup','contact','signup','terms'
			);
