...
$this->msg=t("We have sent bank information instruction to your email")." :$merchant_email";
	...
	public function parish_delivery_prices()
	{
		//dump($this->data);		
		/* print_r($this->data);
		exit;  */
		...
		$delivery_fee = '';
		$services = array();
		if(isset($this->data['deliver_all_parish'])&&($this->data['deliver_all_parish']==2))
		{
			$deliver_all_parish     =  $this->data['deliver_all_parish'];
			$merchant_delivery_type =  isset($this->data['merchant_delivery_type'])?$this->data['merchant_delivery_type']:'';
			$minimum_order_req      =  isset($this->data['minimum_order_req'])?$this->data['minimum_order_req']:''; 	
			$minimum_order_amount   =  isset($this->data['free_delivery_above_price'])?$this->data['free_delivery_above_price']:'';
			$delivery_fee           =  isset($this->data['delivery_fee'])?$this->data['delivery_fee']:'';
		//	$params = array('merchant_id'=>$mtid,'deliver_to_all_parish'=>$deliver_all_parish,'merchant_delivery_type'=>$merchant_delivery_type,'minimum_order_req'=>$minimum_order_req,'minimum_order_amount'=>$minimum_order_amount,'delivery_fee'=>$delivery_fee,'services'=>'');				
		}
		else
		{
			$services = array();
			if(isset($this->data['service_available']))
			{	
				if(sizeof($this->data['service_available'])>0)
				{				
					foreach ($this->data['service_available'] as $key => $parish_id) 
					{
						/* if($this->data['parish_delivery_fee_'.$key]==1)
						{ */
							$delivery_fee               = isset($this->data['delivery_fee_'.$key])?$this->data['delivery_fee_'.$key]:'';
							$chargable_type 			= isset($this->data['parish_delivery_fee_'.$key])?$this->data['parish_delivery_fee_'.$key]:0;
							$minimum_order_req_parish 	= isset($this->data['minimum_order_req_parish_'.$key])?$this->data['minimum_order_req_parish_'.$key]:'';
							$parish_min_amt			 	= isset($this->data['parish_min_amt_'.$key])?$this->data['parish_min_amt_'.$key]:'';
							$services[$key] = array('delivery_fee'=>$delivery_fee,'chargable_type'=>$chargable_type,'minimum_order_req_parish'=>$minimum_order_req_parish,'parish_min_amt'=>$parish_min_amt);
						// }			 
					}				 
					$merchant_delivery_type =  isset($this->data['merchant_delivery_type'])?$this->data['merchant_delivery_type']:'';
					$minimum_order_req      =  isset($this->data['minimum_order_req'])?$this->data['minimum_order_req']:''; 	
					$minimum_order_amount   =  isset($this->data['free_delivery_above_price'])?$this->data['free_delivery_above_price']:'';
					$delivery_fee           =  isset($this->data['delivery_fee'])?$this->data['delivery_fee']:'';
				//	$params = array('merchant_id'=>$mtid,'deliver_to_all_parish'=>'','merchant_delivery_type'=>$merchant_delivery_type,'minimum_order_req'=>$minimum_order_req,'minimum_order_amount'=>$minimum_order_amount,'delivery_fee'=>$delivery_fee,'services'=>json_encode($services));				
				}
			}			
		}
		...
		$params = array('merchant_id'=>$mtid,'deliver_to_all_parish'=>$deliver_all_parish,'merchant_delivery_type'=>$merchant_delivery_type,'minimum_order_req'=>$minimum_order_req,'minimum_order_amount'=>$minimum_order_amount,'delivery_fee'=>$delivery_fee,'services'=>json_encode($services));				
		
		$deals_query = "SELECT * FROM `mt_parish_deliver_settings` WHERE `merchant_id` = ".$mtid." ";		 
		if($res=$DbExt->rst($deals_query))
		{
			//$updating_type = "updateData";				 
			if($this->updateData("{{parish_deliver_settings}}",$params,'merchant_id',$mtid))
				{
					$this->code=1;
		    		$this->msg=Yii::t("default","Setting saved");
				}
		    	...
		    	{
		    		$this->code=2;
		    		$this->msg=Yii::t("default","Error While Updating");
		    	}
		} 			
		else
		{
			if($this->insertData("{{parish_deliver_settings}}",$params))
				{
					$this->code=1;
		    		$this->msg=Yii::t("default","Setting saved");
				}
	...
	public function bookingSummaryReport()
	{
		...
		// print_r(json_decode($_GET['bookingStatus'],true));		  
		...
		if(isset($this->data['bookingStatus'])&&!empty($this->data['bookingStatus']))
		{
			$and .= " AND `status` = '".$this->data['bookingStatus']."'";
		}
		
		$stmt = "SELECT * FROM `mt_bookingtable` WHERE merchant_id=".Yii::app()->functions->q($merchant_id).$and." ORDER BY booking_time ASC, date_booking DESC " ;
		/* $slug=$this->data['slug'];
		$stmt="
	...
	{  
			...
			$feed_data['iTotalDisplayRecords']=$iTotalRecords;
			   ...
			   foreach ($res as $val) {                               
                               ...
                               $image = "<img src='".Yii::app()->getBaseUrl(true)."/upload/".$val['src']."' alt='".$val['img_alt']."'  height='50' width='50'>";
					$action='<div class="options">                                                
                                                <a href="'.$slug.'/Do/Add/?id='.$val['id'].'" >'.Yii::t("default","Edit").'</a>
                                                <a href="javascript:;" class="row_del" rev="'.$val['id'].'">'.Yii::t("default","Delete").'</a>
                                                </div>';		   	   
	...
	{  
			...
			$feed_data['iTotalDisplayRecords']=$iTotalRecords;
			   ...
			   foreach ($res as $val) {                               
			   				  $appending_div = ''; 	
			   				  $merchant_name = '';
			   				  $mechant_list = implode(",",json_decode($val['merchant_list']));
			   				  $stmt1 =" SELECT  `restaurant_name` 
										FROM  {{merchant}} 
										WHERE  `status` =  'active'
										AND  `is_ready` =2
										AND merchant_id IN (".$mechant_list.")
										";                        
							   if ($res1=$this->rst($stmt1))
							   {
									foreach($res1 as $restaurant_name)
									{
										$merchant_name .= $restaurant_name['restaurant_name'] ." , ";
									}								   	 
							   		/* $merchant_name = explode(",",$res1[0]['restaurant_name']);
                               ...
                               $merchant_name = rtrim($merchant_name,' , ');   
					$action='<div class="options">                                                
                                                <a href="'.$slug.'/Do/Add/?id='.$val['id'].'" >'.Yii::t("default","Edit").'</a>                                            
                                                </div>';		   	   
				   /*$date=Yii::app()->functions->prettyDate($val['date_created']);
		...
		public function admin_merchant_list()
	{  
            $slug=$this->data['slug'];            
			$stmt="SELECT * FROM {{admin_enable_deals}}";                        			
			if ($res=$this->rst($stmt)){			
			$iTotalRecords=0;
			$stmt2="SELECT FOUND_ROWS()";
			if ( $res2=$this->rst($stmt2)){
			...
			$feed_data['iTotalDisplayRecords']=$iTotalRecords;
			   ...
			   foreach ($res as $val) {                               
			   				  ...
			   				  $merchant_name = '';
			   				  $mechant_list = implode(",",json_decode($val['merchant_list']));
			   				  $merchant_name_query = " SELECT restaurant_name FROM `mt_merchant` WHERE `merchant_id` IN (".$mechant_list."); ";
			   				  $merchant_name_res=$this->rst($merchant_name_query);
			   				  $merchant_names = '';
			   				  foreach($merchant_name_res as $merchant_name)
			   				  {
			   				  	$merchant_names[] = $merchant_name['restaurant_name'];
			   				  }
			   				  $merchant_names = implode(",",$merchant_names);
                               $status = 'Active';
                               if($val['status']==1)
                               {
                                   $status = 'Inactive';
	...
	public function parish_list()
	{
			...
			if ($res = $this->rst($stmt))
			{
				...
				foreach($res as $val)
				{
					...
					$action = '<div class="options">                                                
                                <a href="' . $slug . '/Do/Add/?id=' . $val['id'] . '" >' . Yii::t("default", "Edit") . '</a>  
                                 <a href="javascript:;" class="row_del" rev="' . $val['id'] . '">' . Yii::t("default", "Delete") . 
                                 '</a> </div>';
					/*$date=Yii::app()->functions->prettyDate($val['date_created']);
					$date=Yii::app()->functions->translateDate($date);*/
					$feed_data['aaData'][] = array(
						ucfirst($val['parish_name']) . $action,						 
						$status
					);
				}
		...
		{	      
		    ...
		    $Validator=new Validator;
			$req=array(
			  'parish_name'=>Yii::t("default","Parish Name is required"),                                         
			);		
			$Validator->required($req,$this->data);
			if ($Validator->validate()){
				$params=array(				  
				  'parish_name'=>$this->data['parish_name'],				  
                  'status'=>$this->data['status']  
				);	
						if(isset($this->data['id'])&&!empty($this->data['id']))
						{
							$res = $this->updateData('{{parish}}',$params,'id',$this->data['id']);  
						}
						else
						{
							$res = $this->insertData("{{parish}}",$params);  	
						}
                                
						if ($res){
							$this->code=1;
			                $this->msg=Yii::t("default",'Parish updated.');  
						}
					 else $this->msg=Yii::t("default","ERROR: cannot update");
	...
	public function merchant_deals_list()
	{

			$mtid=Yii::app()->functions->getMerchantID();			
	        $slug=$this->data['slug'];    
	        $table = $this->data['tbl'];        
			$stmt="SELECT * FROM {{".$table."}} WHERE merchant_id = ".$mtid;                        			
			if ($res=$this->rst($stmt)){			
			$iTotalRecords=0;
			$stmt2="SELECT FOUND_ROWS()";
			if ( $res2=$this->rst($stmt2)){
	...
	{  
			...
			$feed_data['iTotalDisplayRecords']=$iTotalRecords;
			   ...
			   foreach ($res as $val) {                               
                               ...
                               $image = "<img src='".Yii::app()->getBaseUrl(true)."/upload/".$val['img_url']."'   height='50' width='50'>";
					$action='<div class="options">                                                
                                                <a href="'.$slug.'/Do/Add/?id='.$val['id'].'" >'.Yii::t("default","Edit").'</a>
                                                <a href="javascript:;" class="row_del" rev="'.$val['id'].'">'.Yii::t("default","Delete").'</a>
                                                </div>';		   	   
	...
	public function VoucherListNew()
     ...
     {     	
		...
		$stmt="SELECT id,country_code, 
				IF(street = '',concat(city,' ',state,' ',zipcode),concat(street,' ',city,' ',state,' ',zipcode))as address				 
				FROM
				{{address_book}}		
				WHERE
		...
		if ($res=$this->rst($stmt)){
			$address_array = '';
		   foreach ($res as $val) {				   	    			   	    
		   		if(!in_array($val['address'],$address_array))
		   		{
				    ...
				    $action="<div class=\"order-action\">
								<a href=\"javascript:;\" class=\"address_add_btn\" tbl_id=\"$val[id]\"><i class=\"fa fa-pencil-square-o\"></i></a>
								<a href=\"#\" class=\"row_del\" rev=\"$val[id]\"><i class=\"fa fa-trash-o\"></i></a>
							</div>";			 	   	   
			   	/*   $feed_data['aaData'][]=array(
												  $val['address'],
												  $val['location_name'],
     ...
     {     
     	...
     	$address_query = "SELECT * FROM  `mt_address_book` WHERE  `client_id` = ".Yii::app()->functions->getClientId()." AND  `street` LIKE  '%".trim($this->data['street'])."%' AND  `city` LIKE  '%".trim($this->data['city'])."%' AND  `state` LIKE  '%".trim($this->data['state'])."%'";
     	if($res=$this->rst($address_query))
     	{
     		if(isset($res[0]['id']))
     		{
     	...
     	$params=array(
			  ...
			  'state'=>trim($this->data['state']),
			  'zipcode'=>trim($this->data['zipcode']),
			  'location_name'=>trim(isset($this->data['location_name'])?$this->data['location_name']:''),
     	...
     	if ( isset($this->data['add_tbl_update_id']) && $this->data['add_tbl_update_id'] != '' && $this->data['add_tbl_update_id'] != 0){
     		unset($params['date_created']);
     		$params['date_modified']=date('c');
     		if ( $this->updateData("{{address_book}}",$params,'id',$this->data['add_tbl_update_id'])){
     			$this->code=1;
     			$this->msg=Yii::t("default","Successful");		 
     		} else $this->msg=t("ERROR: Something went wrong");	
     	} else {
	...
	{		
		...
		if ($validator->validate()){
			...
			if ( $sms_balance>=1){
				...
				$sms_msg=t("Thanks for the order. Your order reference:")." ".$code;
