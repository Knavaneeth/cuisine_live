...
HTML;
	...
	{		
		...
		/* echo "<pre>";
		print_r($_SESSION['kr_session_item']);
		...
		if (isset($item_details['total'])){
			...
			if(isset($item_details['total']['less_voucher'])&&!empty($item_details['total']['less_voucher']))
			{
			...
			if(isset($item_details['total']['pts_redeem_amt'])&&!empty($item_details['total']['pts_redeem_amt']))
			{
			...
			if(isset($item_details['total']['tips'])&&!empty($item_details['total']['tips']))
			{
	...
	public static function salesReceiptmanual($data='',$item_details='')
	{
		$tr="";
		if (is_array($data) && count($data)>=1){
			foreach ($data as $val) {				
...
<br/><br/>
<p> Thank you for shopping at Cuisine.je , We hope you enjoy your new purchase! Your order number is {receipt-number}. We have included your order receipt and delivery details below:	</p>
<br/>
 {receipt}	
