...
class ScriptManager
{
	...
	{		
        ...
        $curpage = Yii::app()->controller->action->id;
        if($curpage!="signup")
        {
        ...
        $cs->registerScriptFile($baseUrl."/assets/vendor/ui/jquery-ui.js"
		,CClientScript::POS_END); 
		...
		if($curpage=="directions")
		{
			$cs->registerScriptFile($baseUrl."/assets/js/maps.js"
			, CClientScript::POS_END);
			$google_key=getOptionA('google_geo_api_key');
			if (!empty($google_key))
			{
				$cs->registerScriptFile("//maps.googleapis.com/maps/api/js?key=".$google_key."&callback=initMap"
			...
			{
				$cs->registerScriptFile("//maps.googleapis.com/maps/api/js?key=&callback=initMap"
					,CClientScript::POS_END,array(
				  'async'=>'async',
				  'defer'=>'defer'
				)); 
	...
	/*	if($curpage=="signup")
		...
		if($curpage=="profile")
		{
			if(isset($_GET['val'])&&($_GET['val']=="order"))
			{
				$cs->registerScriptFile($baseUrl."/assets/js/datatables.min.js"
				, CClientScript::POS_END);		
			}
		}  
		...
		if($curpage=="menu")
		{
			...
			$merchant_id = Yii::app()->functions->get_merchant_details($merchant_slug);		
		 	...
		 	{ */
		 		...
		 		/* $cs->registerScriptFile($baseUrl."/assets/js/images-grid.js"
				, CClientScript::POS_END);		
			 
				$cs->registerScriptFile($baseUrl."/assets/js/image_grid.js"
				, CClientScript::POS_END);		 */

				$cs->registerScriptFile($baseUrl."/assets/js/picturefill.min.js"
				, CClientScript::POS_END);		
			 
				$cs->registerScriptFile($baseUrl."/assets/js/lightgallery-all.min.js"
				, CClientScript::POS_END);

				$cs->registerScriptFile($baseUrl."/assets/js/jquery.mousewheel.min.js"
				, CClientScript::POS_END);
		 /*	}		 	  */
		}  
	...
	/*	if ( Yii::app()->functions->getOptionAdmin('fb_flag')=="" ){
			$cs->registerScriptFile($baseUrl."/assets/js/fblogin.js"
		    ,CClientScript::POS_END); 
		}  */
		...
		/*$cs->registerScriptFile($baseUrl."/assets/js/bootstrap-datepicker.js"
		,CClientScript::POS_END); */
		
		$cs->registerScriptFile($baseUrl."/assets/js/bootstrap-timepicker.js"
		,CClientScript::POS_END); 
		
		$cs->registerScriptFile($baseUrl."/assets/vendor/jquery-cookie.js"
		,CClientScript::POS_END); 
		
		$cs->registerScriptFile($baseUrl."/assets/js/date.js"
		,CClientScript::POS_END); 		
		...
		$cs->registerScriptFile($baseUrl."/assets/js/jquery.flexnav.js"
		,CClientScript::POS_END); 
		...
		$cs->registerScriptFile($baseUrl."/assets/js/jquery.superslides.js"
		,CClientScript::POS_END); 
		
		$cs->registerScriptFile($baseUrl."/assets/js/scripts.js"
		,CClientScript::POS_END); 
		
		  $cs->registerScriptFile($baseUrl."/assets/js/cropper.min.js"
		,CClientScript::POS_END); 
                
               $cs->registerScriptFile($baseUrl."/assets/js/cropper_main.js"
		,CClientScript::POS_END);   
		
		$cs->registerScriptFile($baseUrl."/assets/js/jquery.newsTicker.js"
		,CClientScript::POS_END);   
	}
	...
	public static function registerAllCSSFiles()
	{
		...
		$cs->registerCssFile($baseUrl.'/assets/vendor/ui/jquery-ui.css');
		
		$cs->registerCssFile("//fonts.googleapis.com/css?family=Open+Sans|PT+Sans|Source+Sans+Pro:400,600,700,300|Roboto|Montserrat:400,700|Lato:400,300,100italic,100,300italic,400italic,700,700italic,900,900italic|Raleway:300,400,600,800");					
		...
		$cs->registerCssFile($baseUrl.'/assets/css/date_time_picker.css');
		/*$cs->registerCssFile($baseUrl."/assets/vendor/timepicker.co/jquery.ui.timepicker.css");*/
		$cs->registerCssFile($baseUrl.'/assets/css/owl.carousel.css');
		$cs->registerCssFile($baseUrl.'/assets/css/owl.theme.css');
		$cs->registerCssFile($baseUrl.'/assets/css/owl.transitions.css');
		$cs->registerCssFile($baseUrl.'/assets/css/superslides.css');
		$cs->registerCssFile($baseUrl.'/assets/css/style.css');
		$cs->registerCssFile($baseUrl.'/assets/css/responsive.css');
		// $cs->registerCssFile($baseUrl.'/assets/css/bootstrap-theme.min.css');
		// $cs->registerCssFile($baseUrl.'/assets/css/site.css');
		$curpage = Yii::app()->getController()->getAction()->controller->action->id;
		...
		{			 
			...
			$merchant_id = Yii::app()->functions->get_merchant_details($merchant_slug);				 
		 	if(in_array(Yii::app()->functions->get_merchant_service($merchant_id),$booking_array))
		 	{*/
			//	$cs->registerCssFile($baseUrl.'/assets/css/images-grid.css');			 			 
		 		$cs->registerCssFile($baseUrl.'/assets/css/lightgallery.css');			 			 
			// }
		}
		...
		if($curpage=="profile")
		{
			if(isset($_GET['val'])&&($_GET['val']=="order"))
			{
				$cs->registerCssFile($baseUrl."/assets/css/datatables.min.css");		
			}
		}
