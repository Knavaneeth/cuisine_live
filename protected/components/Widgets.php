...
class Widgets extends CApplicationComponent
{
	...
	public static function applyVoucher($merchant_id='')
	{
		...
		?>
		<div class="voucher_wrap row row-sm">                      
		<div class="col-md-8">                      
        ...
        <div class="col-md-4"><a href="javascript:;" class="btn btn-primary btn-block apply_voucher"><?php echo $text?></a></div>         
        </div>
		<?php
	}
 ...
 <?php echo Yii::t("default","View Map")?></a>
	...
	public static function AddressByMap()
	{
		...
		$address_list=Yii::app()->functions->client_addressBook(Yii::app()->functions->getClientId());			
		 
		// echo " Hi ".count($address_list); exit;
		if(count($address_list)==1)
		{				
			foreach ($address_list as $address_list_value)
			{
				 if($address_list_value['map_address']!='')
				 {
				 	$default_address[] = $address_list_value['map_address'];	
				 }
			}			
		}
		else if(count($address_list)>1)
		{
			foreach ($address_list as $key => $address_list_value)
			{				
				$default_address[] = $address_list_value['map_address'];	
				if($address_list_value['as_default']==2)
				{
					$default_address[0] =  $address_list_value['map_address'];	
					break;
				}
			}
			...
			/*	if($key!='as_default')
				{
					$default_address[] = $address_list_value;	
					if(isset($address_list['as_default'])&&$address_list['as_default']==2)
					{
						$default_address[0] =  $address_list[$key];
						break;
					}
				}  */
		}
		...
		// echo CHtml::hiddenField('temporary_address',isset($_SESSION['kr_search_address'])?$_SESSION['kr_search_address']:'Digital Jersey Hub  Block 3,Ground Floor,Grenville St,Jersey,St Helier JE2 4UF');
		echo CHtml::hiddenField('temporary_address',isset($default_address[0])?$default_address[0]:'');
