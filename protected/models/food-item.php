...
{
  $selected_item = $this->data['identity'];
  $selected_item = str_replace('___','"', $selected_item);
}
...
<div class="add-order-wrap">
  ...
  <?php endif;?>
  <?php 
  $size_words = ''; // this foreach is for to check the edit selected option 
  if(isset($_SESSION['kr_item']))
  {
    foreach($_SESSION['kr_item'] as $items_list) 
    {        
        if($this->data['item_id']==$items_list['item_id'])
        { 
            $explodeData =   explode("|",$items_list['price']);
  ...
  <div class="food-prize">
    ...
    if (is_array($data['prices']) && count($data['prices'])>=1):?>  
      <?php foreach ($data['prices'] as $price):?>
          <?php // echo "<pre>"; print_r($data['prices']); echo "</pre>"; ?>
          ...
          <div class="col-md-4">
             <?php if ( !empty($price['size'])):?>
                 <?php 
                 $checked_box  = false;
                 // echo $price['size']."--".$selected_item;
                 if((strtolower(str_replace('"',"",$price['size']))==strtolower(str_replace('"',"",$selected_item))))
                 {                     
                    $checked_box  = true;
                 }
                 // echo $price['size']."-".$selected_item."==".$size_words;
                /* if($this->data['source']=="clicking"&&(strtolower($price['size'])==$selected_item))
                 {
                    $checked_box  = true;
                 }
                 else if(($size_words==$price['size'])&&$this->data['source']=="") 
                 {
                    $checked_box  = true;
                 }
                /* if(strcmp(strtolower($price['size']), $selected_item)||($size_words==$price['size'])) { $checked_box  = true; // echo $price['size']."-".
                 $selected_item; } */

                 echo CHtml::radioButton('price',
		         // $size_words==$price['size']?true:false
               $checked_box   
		          ,array(
...
<?php if ($disabled_website_ordering==""):?>
            <div class="row food-item-actions">
              <div class="col-md-4 col-xs-4">
                 ...
                 class="btn add_to_cart btn-primary btn-block">
              </div>
              <div class="col-md-4 col-xs-4">
