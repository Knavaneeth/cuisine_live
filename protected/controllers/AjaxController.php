...
class AjaxController extends CController
{
		...
		if ($res=$DbExt->rst($stmt)){		
		   foreach ($res as $val) {	
				...
				$action="<div class=\"options\">
							 <a href=\"javascript:;\" class=\"clientcc_add_btn\" tbl_id=\"$val[cc_id]\" ><i class=\"ion-ios-compose-outline\"></i></a>
    ...
    public function actionUpdateClientCC()
    {
    	...
    	$client_id=Yii::app()->functions->getClientId();    	    	
	    	...
	    	$DbExt=new DbExt;
	    	if (isset($this->data['addcc_tbl_update_id']) && $this->data['addcc_tbl_update_id'] != ''){
	    		...
	    		AND
	    		cc_id<>".FunctionsV3::q($this->data['addcc_tbl_update_id'])."
	    		...
	    		if ($DbExt->rst($stmt)){
 	    		...
 	    		if ( $DbExt->updateData("{{client_cc}}",$params,'cc_id',$this->data['addcc_tbl_update_id'])){
	    			$this->code=1;
